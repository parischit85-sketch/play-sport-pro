...
export default function MobileRankingChart({
  ...
  const visiblePlayers = showAll
    ? seriesKeys
    : seriesKeys.slice(0, selectedCount);
    ...
    if (active && payload && payload.length > 0) {
      return (
        <div className="bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-3 max-w-xs">
          <p
            className={`font-semibold text-gray-900 dark:text-gray-100 mb-2 ${isMobile ? "text-sm" : "text-base"}`}
          >
            {label}
          </p>
            ...
            {payload.map((entry, index) => (
              <div key={index} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span
                    className={`text-gray-600 dark:text-gray-400 truncate ${isMobile ? "text-xs max-w-20" : "text-sm max-w-28"}`}
                  >
    ...
    <div className="space-y-4">
          ...
          >
            ...
            {activeLines.map((key, index) => (
              <Line
                ...
                dataKey={key}
                stroke={
                  PODIUM_COLORS[seriesKeys.indexOf(key)] ||
                  `hsl(${index * 45}, 70%, 50%)`
                }
                strokeWidth={
                  selectedPlayers.has(key)
                    ? isMobile
                      ? 3
                      : 4
                    : isMobile
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h4
            className={`font-medium text-gray-700 dark:text-gray-300 ${isMobile ? "text-sm" : "text-base"}`}
          >
        ...
        >
          {visiblePlayers.map((playerName, index) => {
            ...
            const color =
              PODIUM_COLORS[seriesKeys.indexOf(playerName)] ||
              `hsl(${index * 45}, 70%, 50%)`;
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div
                    className={`font-medium text-gray-900 dark:text-gray-100 truncate ${isMobile ? "text-xs" : "text-sm"}`}
                  >
