...
export default function ModernMultiLineChart({
  ...
  const yAxisDomain = useMemo(() => {
    if (last10Data.length === 0 || seriesKeys.length === 0)
      return ["auto", "auto"];
    ...
    return (
      <div className="flex justify-center gap-2 md:gap-3 mt-3 md:mt-4 flex-wrap">
        {sortedPayload.map((entry, index) => {
          ...
          const playerRanking = playerRankings.find(
        ...
        >
          ...
          {seriesKeys.map((key, index) => (
            <Line
              ...
              className={
                index < 3 ? "drop-shadow-md" : index < 5 ? "drop-shadow-sm" : ""
              }
            />
          ))}
        </LineChart>
