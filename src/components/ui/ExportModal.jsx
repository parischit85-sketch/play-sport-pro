...
export default function ExportModal({ isOpen, onClose, players, filteredPlayers, T }) {
  ...
  const exportColumns = [
    ...
    {
      key: 'tournamentData.isParticipant',
      label: 'Partecipa Torneo',
      formatter: formatBooleanForExport,
    },
  ];
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Esporta Giocatori" size="medium">
      <div className="space-y-6">
        {/* Opzioni di esportazione */}
        <div className={`${T.cardBg} ${T.border} rounded-lg p-4`}>
          <h3 className={`text-lg font-medium mb-4 ${T.text}`}>Scegli cosa esportare</h3>
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center space-x-3">
              ...
              />
              <div>
                <div className={`font-medium ${T.text}`}>Tutti i giocatori ({players.length})</div>
                <div className={`text-sm ${T.subtext}`}>Esporta tutti i giocatori senza filtri</div>
              </div>
            </label>
          </div>
        </div>
        ...
        {/* Informazioni */}
        <div className={`text-sm ${T.subtext} bg-blue-50 bg-blue-900/20 p-3 rounded-lg`}>
          <div className="flex items-start space-x-2">
            <span className="text-blue-500 mt-0.5">ℹ️</span>
            <div>
              Verrà scaricato un file CSV contenente {dataToExport.length} giocatori. Il file
              includerà tutte le informazioni principali di ogni giocatore.
            </div>
          </div>
        </div>
