...
export class PushService {
  ...
  _isTestEnv() {
    try {
      if (typeof import.meta !== 'undefined' && (import.meta.vitest || (import.meta.env && import.meta.env.MODE === 'test'))) return true;
  } catch { /* noop */ }
    const g = typeof globalThis !== 'undefined' ? globalThis : {};
    return !!(
      (g.process && g.process.env && (g.process.env.NODE_ENV === 'test' || g.process.env.VITEST_WORKER_ID)) ||
      g.__vitest_worker__ || g.vi
    );
  }
  ...
  async sendWithRetry(userId, notification, options = {}, attempt = 1) {
    try {
      ...
      for (const subscription of subscriptions) {
        try {
          ...
          if (error.statusCode === 429 && attempt <= this.maxRetries) {
            ...
            console.warn(`⏳ [PushService] Rate limited, retrying in ${backoffMs}ms (attempt ${attempt}/${this.maxRetries})`);
          ...
          if (error.statusCode >= 500 && error.statusCode < 600 && attempt <= this.maxRetries) {
            this.metrics.retried++;
            const backoffMs = this.calculateBackoff(attempt);
            console.warn(`⚠️ [PushService] Server error ${error.statusCode}, retrying in ${backoffMs}ms`);
            await this.sleep(backoffMs);
            return this.sendWithRetry(userId, notification, options, attempt + 1);
          }
      ...
      if (attempt >= this.maxRetries) {
        console.error(`❌ [PushService] Max retries (${this.maxRetries}) exceeded for user ${userId}`);
      ...
      this.metrics.retried++;
      const backoffMs = this.calculateBackoff(attempt);
      console.warn(`🔄 [PushService] Retrying after ${backoffMs}ms (attempt ${attempt}/${this.maxRetries})`);
      await this.sleep(backoffMs);
      return this.sendWithRetry(userId, notification, options, attempt + 1);
    }
  }
