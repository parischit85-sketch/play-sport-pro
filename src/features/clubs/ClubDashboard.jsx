...
const QuickActionCard = ({ title, description, icon, onClick, gradient, iconBg }) => (
  <button onClick={onClick} className={'relative group overflow-hidden rounded-xl md:rounded-2xl p-3 md:p-6 transition-all duration-300 hover:scale-105 hover:shadow-2xl ' + gradient + ' border border-white/20 dark:border-gray-700/30'}>
    <div className="absolute inset-0 bg-gradient-to-br from-white/10 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
    <div className="relative z-10">
      <div className={'w-10 h-10 md:w-14 md:h-14 rounded-lg md:rounded-xl ' + iconBg + ' flex items-center justify-center mb-2 md:mb-4 shadow-lg transform group-hover:scale-110 transition-transform duration-300'}>
        {icon}
      </div>
...
const ClubDashboard = () => {
  ...
  useEffect(() => {
    if (user && clubId) {
      const loadActiveBookings = async () => {
        ...
        try {
          console.log('🔍 [ClubDashboard] Loading bookings for user in club:', { userId: user.uid, clubId });
  ...
  const refreshBookings = useCallback(async () => {
    if (user && clubId) {
      setLoadingBookings(true);
      try {
        console.log('🔄 [ClubDashboard] Refreshing bookings for user in club:', { userId: user.uid, clubId });
  ...
  if (clubLoading) {
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-gray-800 flex items-center justify-center"><LoadingSpinner size="lg" /></div>);
  }

  if (!club) {
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-gray-800 flex items-center justify-center"><div className="text-center"><div className="text-6xl mb-4">❌</div><h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Club non trovato</h1><button onClick={() => navigate('/clubs/search')} className="mt-4 bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg">Cerca altri club</button></div></div>);
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-gray-800 py-4 md:py-6 px-4">
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
        <div className="hidden lg:block text-center mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400">📍 {club.location?.city}, {club.location?.region}</p>
        </div>
          ...
          {loadingBookings ? (
            <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-2xl p-6 md:p-8">
              <div className="space-y-3">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="bg-gray-100 dark:bg-gray-700 p-4 rounded-xl animate-pulse">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-[10px] text-gray-600 dark:text-gray-400 truncate mb-1">
                              {isLessonBooking ? (
                                <>
                                  {booking.instructor && (
                                    <span>Maestro: {booking.instructorName || booking.instructor}</span>
                                  )}
                                  {booking.lessonType && !booking.instructor && (
                                    <span>{booking.lessonType === 'individual' ? 'Individuale' : 'Gruppo'}</span>
                                  )}
                                </>
                              ) : (
                                <>
                ...
                <div className="flex justify-center mt-3 md:hidden">
                  <div className="flex gap-1">
                    {activeBookings.slice(0, Math.min(6, activeBookings.length)).map((_, index) => (
                      <div key={index} className="w-1 h-1 rounded-full bg-gray-300/60 dark:bg-gray-600/60"></div>
                    ))}
            ...
            <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-2xl p-4 md:p-8 text-center">
              ...
              <h3 className="text-base md:text-xl font-bold text-gray-900 dark:text-white mb-2">Nessuna prenotazione attiva</h3>
              <p className="text-sm md:text-base text-gray-600 dark:text-gray-400 mb-3 md:mb-4">Non hai prenotazioni future in questo club</p>
              <button onClick={() => navigate(`/club/${clubId}/booking`)} className="bg-blue-500 hover:bg-blue-600 text-white px-4 md:px-6 py-2 md:py-3 rounded-lg font-medium text-sm md:text-base">Prenota un Campo</button>
            </div>
          )}
        </div>
        <div className="space-y-3 md:space-y-4">
          <h2 className="text-lg md:text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2 md:gap-3">
            <div className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center"><svg className="w-5 h-5 md:w-6 md:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg></div>
            Azioni Rapide
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
            <QuickActionCard title="Prenota Campo" description="Prenota un campo da gioco" icon={<svg className="w-5 h-5 md:w-7 md:h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>} onClick={() => navigate(`/club/${clubId}/booking`)} gradient="bg-gradient-to-br from-emerald-500 to-green-600" iconBg="bg-white/20 backdrop-blur-sm" />
            <QuickActionCard title="Prenota Lezione" description="Prenota una lezione" icon={<svg className="w-5 h-5 md:w-7 md:h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>} onClick={() => navigate(`/club/${clubId}/lessons`)} gradient="bg-gradient-to-br from-blue-500 to-indigo-600" iconBg="bg-white/20 backdrop-blur-sm" />
            <QuickActionCard title="Classifica" description="Ranking RPA" icon={<svg className="w-5 h-5 md:w-7 md:h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>} onClick={() => navigate(`/club/${clubId}/classifica`)} gradient="bg-gradient-to-br from-amber-500 to-orange-600" iconBg="bg-white/20 backdrop-blur-sm" />
            <QuickActionCard title="Statistiche" description="Grafici avanzati" icon={<svg className="w-5 h-5 md:w-7 md:h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 8v8m-4-5v5m-4-2v2m-2 4h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>} onClick={() => navigate(`/club/${clubId}/stats`)} gradient="bg-gradient-to-br from-purple-500 to-pink-600" iconBg="bg-white/20 backdrop-blur-sm" />
          </div>
        </div>
      </div>
