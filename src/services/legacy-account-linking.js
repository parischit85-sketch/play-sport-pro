  ...
  try {
    ...
    if (matches.length === 0 && userDisplayName) {
      ...
      profilesSnapshot.forEach(doc => {
        ...
        if (similarity > 70) { // Soglia di similarità
          matches.push({
            type: 'name_similar',
    ...
    if (matches.length > 0) {
      console.log(`   🎯 Miglior match: ${matches[0].profile.displayName} (${matches[0].confidence}%)`);
    }
...
export async function linkNewAccountToLegacyProfile(firebaseUser, legacyProfile) {
  console.log(`🔗 Collegando account ${firebaseUser.email} a profilo esistente ${legacyProfile.displayName}`);
