...
export function FormulaExplainer({
  ...
  return (
    <div className="text-sm space-y-3 leading-6">
      ...
      <div>
        Gap (S<sub>opp</sub> − S<sub>you</sub>) = <b>{Math.round(gap)}</b> •
        Fascia: {rpaBracketText(gap)}
      </div>
      <ol className="list-decimal pl-5 space-y-1">
        <li>
          <b>Base</b> = (Somma ranking coppie) / 100 = ({Math.round(sumA)} +{" "}
          {Math.round(sumB)}) / 100 = <b>{baseRaw.toFixed(2)}</b>
        </li>
        <li>
          <b>DG</b> (Differenza Game) dal punto di vista della squadra vincente
          = <b>{GD}</b>
        </li>
        <li>
          <b>Fattore</b> dalla tabella fasce in base al Gap ={" "}
          <b>{factor.toFixed(2)}</b>
        </li>
        <li>
          <b>Punti</b> = (Base + DG) × Fattore = (<b>{baseRaw.toFixed(2)}</b> +{" "}
          <b>{GD}</b>) × <b>{factor.toFixed(2)}</b> ={" "}
          <b>{finalPoints.toFixed(2)}</b>
        </li>
        <li>
