  ...
  if (filteredCourts.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <div className="font-medium text-gray-600 dark:text-gray-400">Nessun campo disponibile</div>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between p-4">
          ...
          <div className="flex-1 text-center">
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              {court?.hasHeating && (
                <span className="text-xs px-3 py-1 rounded-full bg-orange-100 dark:bg-orange-900 text-orange-700 dark:text-orange-300 font-medium">
                  🔥 Riscaldamento
                </span>
              )}
            </div>
          ...
          {filteredCourts.map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all ${
                index === activeCourt ? 'w-8 bg-blue-500' : 'w-2 bg-gray-300 dark:bg-gray-600'
              }`}
            />
          ))}
        </div>
      ...
      <div className={`sticky bottom-0 ${T.cardBg} border-t-2 ${T.border} p-3 shadow-lg`}>
        <div className="flex items-center justify-around text-xs">
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded bg-emerald-200 dark:bg-emerald-800 border border-emerald-400" />
            <span className="text-gray-600 dark:text-gray-400">Libero</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded bg-blue-200 dark:bg-blue-800 border border-blue-400" />
            <span className="text-gray-600 dark:text-gray-400">Occupato</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded bg-gray-200 dark:bg-gray-800 border-dashed border border-gray-400" />
            <span className="text-gray-600 dark:text-gray-400">Non disp.</span>
          </div>
        </div>
      </div>
    </div>
