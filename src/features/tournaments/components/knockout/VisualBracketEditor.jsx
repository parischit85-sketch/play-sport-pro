    ...
    return (
      <div className="w-full">
        <button
          ...
          className="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-left px-3 py-2 text-sm text-gray-900 dark:text-gray-100 hover:border-primary-400 focus:outline-none focus:ring-2 focus:ring-primary-300"
          title="Seleziona dalla classifica del girone"
        >
          {value === '' && <span className="text-gray-400">— {label} —</span>}
          {value === 'BYE' && <span className="text-amber-700 dark:text-amber-300">BYE</span>}
          {value && value !== 'BYE' && (
            <span>
              {current ? `${current.teamName} · Girone ${current.groupId} · Pos ${current.position}` : value}
            </span>
          )}
        </button>
      </div>
  ...
  const renderFirstRound = () => (
    <div className="flex flex-col gap-6 min-w-[260px]">
      ...
      {firstRoundPairs.map(([a, b], idx) => (
        <div key={idx} className="relative bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 shadow-sm">
          <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Match {idx + 1}</div>
    ...
    return (
      <div className="flex flex-col gap-6 min-w-[220px]">
        ...
        {Array.from({ length: currCount }).map((_, idx) => (
          <div key={idx} className="bg-gray-50 dark:bg-gray-900/40 border border-dashed border-gray-300 dark:border-gray-700 rounded-lg p-4 text-center text-xs text-gray-400">
            Match {idx + 1}
          </div>
        ))}
      </div>
        ...
        <div className="inline-flex gap-4 md:gap-8 p-2 md:p-3">
          ...
          {rounds.slice(1).map((_, i) => (
            <React.Fragment key={i}>
                <div className="flex items-center"><ChevronRight className="w-6 h-6 md:w-8 md:h-8 text-gray-300 dark:text-gray-600"/></div>
...
function RoundHeader({ title, count }) {
  return (
    <div className="sticky top-0 z-10 bg-gradient-to-r from-primary-100 to-blue-100 dark:from-primary-900/40 dark:to-blue-900/40 rounded-lg px-4 py-2 border border-primary-200 dark:border-primary-800">
      <div className="flex items-center gap-2 justify-center">
        <Crown className="w-4 h-4 text-primary-600" />
        <h4 className="text-sm font-semibold text-gray-900 dark:text-white">{title}</h4>
      </div>
      <div className="text-[11px] text-gray-600 dark:text-gray-400 text-center mt-1">{count} {count === 1 ? 'partita' : 'partite'}</div>
    </div>
  );
}
...
function buildRounds(totalSlots, startingRound) {
  ...
  if (startingRound === KNOCKOUT_ROUND.ROUND_OF_16) {
    seq.push({ code: KNOCKOUT_ROUND.ROUND_OF_16, label: mapLabel(KNOCKOUT_ROUND.ROUND_OF_16), matches: firstMatches });
    seq.push({ code: KNOCKOUT_ROUND.QUARTER_FINALS, label: mapLabel(KNOCKOUT_ROUND.QUARTER_FINALS), matches: Math.max(1, firstMatches / 2) });
    seq.push({ code: KNOCKOUT_ROUND.SEMI_FINALS, label: mapLabel(KNOCKOUT_ROUND.SEMI_FINALS), matches: Math.max(1, firstMatches / 4) });
    ...
    seq.push({ code: KNOCKOUT_ROUND.QUARTER_FINALS, label: mapLabel(KNOCKOUT_ROUND.QUARTER_FINALS), matches: firstMatches });
    seq.push({ code: KNOCKOUT_ROUND.SEMI_FINALS, label: mapLabel(KNOCKOUT_ROUND.SEMI_FINALS), matches: Math.max(1, firstMatches / 2) });
    seq.push({ code: KNOCKOUT_ROUND.FINALS, label: mapLabel(KNOCKOUT_ROUND.FINALS), matches: 1 });
  } else if (startingRound === KNOCKOUT_ROUND.SEMI_FINALS) {
    seq.push({ code: KNOCKOUT_ROUND.SEMI_FINALS, label: mapLabel(KNOCKOUT_ROUND.SEMI_FINALS), matches: firstMatches });
    seq.push({ code: KNOCKOUT_ROUND.FINALS, label: mapLabel(KNOCKOUT_ROUND.FINALS), matches: 1 });
