 ...
 */
export async function advancePhaseWithTransaction(clubId, tournamentId, newStatus, additionalData = {}) {
  try {
...
export async function submitMatchResultWithTransaction(clubId, tournamentId, matchId, resultData) {
  try {
    const result = await runTransaction(db, async (transaction) => {
      const matchRef = doc(db, 'clubs', clubId, COLLECTIONS.TOURNAMENTS, tournamentId, COLLECTIONS.MATCHES, matchId);
...
export async function rollbackToPreviousPhase(clubId, tournamentId) {
  try {
    ...
    if ((lastTransition.to === TOURNAMENT_STATUS.GROUPS_GENERATION || lastTransition.to === TOURNAMENT_STATUS.GROUPS_PHASE) && previousStatus === TOURNAMENT_STATUS.REGISTRATION_CLOSED) {
      ...
      const teamsColl = collection(db, 'clubs', clubId, COLLECTIONS.TOURNAMENTS, tournamentId, COLLECTIONS.TEAMS);
    ...
    } else if (lastTransition.to === TOURNAMENT_STATUS.GROUPS_PHASE && previousStatus === TOURNAMENT_STATUS.REGISTRATION_CLOSED) {
    ...
    if (lastTransition.to === TOURNAMENT_STATUS.KNOCKOUT_PHASE && previousStatus === TOURNAMENT_STATUS.GROUPS_PHASE) {
