...
export default function MobileRankingChart({
  ...
  const activeLines = showAll
    ? visiblePlayers.filter((key) => selectedPlayers.has(key))
    : visiblePlayers;
  ...
  const ResponsiveTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length > 0) {
      return (
        <div className="bg-white/95 bg-gray-800/95 backdrop-blur-sm border border-gray-200 border-gray-700 rounded-lg shadow-lg p-3 max-w-xs">
          <p
            className={`font-semibold text-gray-900 text-gray-100 mb-2 ${isMobile ? 'text-sm' : 'text-base'}`}
          >
            {label}
          </p>
            ...
            {payload.map((entry, index) => (
              <div key={index} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-gray-600 text-gray-400 truncate ${isMobile ? 'text-xs max-w-20' : 'text-sm max-w-28'}`}
                  >
    ...
    <div className="space-y-4">
      ...
      <div className={`${isMobile ? 'h-64' : 'h-80 md:h-96'}`}>
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            {!isMobile && (
              <Legend wrapperStyle={{ fontSize: '12px', paddingTop: '15px' }} iconType="line" />
            )}
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h4
            className={`font-medium text-gray-700 text-gray-300 ${isMobile ? 'text-sm' : 'text-base'}`}
          >
            Giocatori da mostrare:
          </h4>
          <button
            onClick={() => setShowAll(!showAll)}
            className={`px-3 py-1 bg-blue-100 bg-blue-900/20 text-blue-700 text-blue-300 rounded-full transition-colors ${isMobile ? 'text-xs' : 'text-sm'}`}
          >
        ...
        <div
          className={`grid gap-2 ${isMobile ? 'grid-cols-2' : 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4'}`}
        >
          {visiblePlayers.map((playerName, index) => {
            const isSelected = selectedPlayers.has(playerName);
            const color =
              PODIUM_COLORS[seriesKeys.indexOf(playerName)] || `hsl(${index * 45}, 70%, 50%)`;
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 p-2 rounded-lg border transition-all text-left ${
                  isSelected
                    ? 'border-blue-300 bg-blue-50 bg-blue-900/20 shadow-sm'
                    : 'border-gray-200 border-gray-700 hover:bg-gray-50 hover:bg-gray-800/50'
                }`}
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div
                    className={`font-medium text-gray-900 text-gray-100 truncate ${isMobile ? 'text-xs' : 'text-sm'}`}
                  >
      ...
      <div
        className={`flex justify-between items-center text-gray-500 ${isMobile ? 'text-xs' : 'text-sm'}`}
      >
        <span>
          Ultimi {chartData.length} {isMobile ? 'periodi' : 'punti dati'}
        </span>
