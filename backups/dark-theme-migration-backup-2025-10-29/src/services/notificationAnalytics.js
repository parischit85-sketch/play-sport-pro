...
class NotificationAnalytics {
   ...
   * @param {Object} params - Parametri evento
   */
  async trackDelivered({ notificationId, userId, channel, latency, cost = 0, retryCount = 0 }) {
    const event = {
   ...
   * @param {Object} params - Parametri evento
   */
  async trackChannelFallback({ notificationId, userId, fromChannel, toChannel, reason }) {
    const event = {
   ...
   * @param {Object} params - Parametri evento
   */
  async trackABTestAssignment({ abTestId, userId, variant, notificationId }) {
    const event = {
  ...
  async getChannelPerformance(startDate, endDate) {
    try {
      ...
      return Object.entries(channelStats)
        .map(([channel, stats]) => {
          const deliveryRate =
            stats.sent > 0 ? ((stats.delivered / stats.sent) * 100).toFixed(2) : '0.00';

          const errorRate =
            stats.sent > 0 ? ((stats.failed / stats.sent) * 100).toFixed(2) : '0.00';

          const ctr =
            stats.delivered > 0 ? ((stats.clicked / stats.delivered) * 100).toFixed(2) : '0.00';
  ...
  async getABTestResults(abTestId) {
    try {
      ...
      const q = query(eventsRef, where('abTestId', '==', abTestId));
      ...
      const results = Object.entries(variantStats).map(([variant, stats]) => {
        ...
        const conversionRate =
          stats.clicked > 0 ? ((stats.converted / stats.clicked) * 100).toFixed(2) : '0.00';
  ...
  async getDashboardMetrics(lastMinutes = 60) {
    try {
      ...
      // Calcola rates
      const deliveryRate =
        metrics.sent > 0 ? ((metrics.delivered / metrics.sent) * 100).toFixed(2) : '0.00';

      const errorRate =
        metrics.sent > 0 ? ((metrics.failed / metrics.sent) * 100).toFixed(2) : '0.00';

      const ctr =
        metrics.delivered > 0 ? ((metrics.clicked / metrics.delivered) * 100).toFixed(2) : '0.00';

      const conversionRate =
        metrics.clicked > 0 ? ((metrics.converted / metrics.clicked) * 100).toFixed(2) : '0.00';
  ...
  async _updateAggregateStats(eventType, data) {
    try {
      ...
      if (eventType === 'delivered') {
        ...
        updates[`notifications.${data.notificationId}.revenue`] = increment(
