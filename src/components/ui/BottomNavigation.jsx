...
export default function BottomNavigation({
  ...
  return (
    <div
      className="md:hidden bottom-nav-container bg-gray-900/95 backdrop-blur-2xl border-t border-gray-700/50 shadow-2xl shadow-black/20"
      style={{
        zIndex: 999999,
    ...
    >
      ...
      {showClubMenu && isAdmin && hamburgerMenuItems.length > 0 && (
        <div
          className="club-menu-container absolute bottom-full left-0 right-0 bg-gray-900/95 backdrop-blur-2xl border-t border-gray-700/30 shadow-2xl shadow-black/40"
          ...
          >
            <div className="flex justify-between items-center mb-3 sm:mb-4 sticky top-0 bg-white/95/95 backdrop-blur-xl z-10 pb-2">
              ...
              <button
                ...
                className="w-8 h-8 rounded-full bg-gray-700/60 backdrop-blur-sm border border-gray-600/30 hover:bg-gray-600/80 text-gray-300 hover:text-white transition-all duration-300 flex items-center justify-center shadow-lg hover:shadow-xl transform hover:scale-105"
              >
              ...
              {hamburgerMenuItems.map((item) => (
                <div
                  ...
                  className={`flex items-center space-x-2 sm:space-x-3 p-3 sm:p-3.5 rounded-lg sm:rounded-xl cursor-pointer transition-all duration-300 transform hover:scale-105 min-h-[56px] ${
                    active === item.id
                      ? 'bg-gradient-to-r from-blue-400/20 to-indigo-400/20 text-blue-400 shadow-lg shadow-blue-900/20 border border-blue-600/30'
                      : 'bg-gray-700/60 backdrop-blur-sm text-gray-300 hover:bg-gray-600/80 border border-gray-600/30 shadow-lg hover:shadow-xl'
                  }`}
            ...
            >
              ...
              <div
                className={`relative flex items-center justify-center w-11 h-11 rounded-2xl transition-all duration-300 ${
                  active === item.id
                    ? 'bg-gradient-to-br from-blue-400 to-indigo-500 shadow-lg shadow-blue-900/30'
                    : 'hover:bg-gradient-to-br hover:from-gray-700 hover:to-gray-600 hover:shadow-md'
                }`}
              >
                <div
        ...
        {isAdmin && hamburgerMenuItems.length > 0 && (
          ...
          >
            <div
              className={`relative flex items-center justify-center w-11 h-11 rounded-2xl transition-all duration-300 ${
                showClubMenu
                  ? 'bg-gradient-to-br from-purple-400 to-pink-500 shadow-lg shadow-purple-900/30'
                  : 'hover:bg-gradient-to-br hover:from-gray-700 hover:to-gray-600 hover:shadow-md'
              }`}
            >
              <svg
                className={`w-5 h-5 ${showClubMenu ? 'text-white scale-110' : 'text-gray-400'} transition-all duration-300`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
