...
function MatchResultModal({ match, team1, team2, onClose, onSubmit, T: externalT }) {
  ...
  const updateSetScore = (index, teamKey, value) => {
    // Permetti stringa vuota (nessun valore inserito) o numero >= 0
    const val = value === '' ? '' : Math.max(0, Number.isNaN(Number(value)) ? 0 : Number(value));
    setSets((prev) => {
      const next = [...prev];
  ...
  const addSet = () => {
    setSets((prev) => (prev.length < 3 ? [...prev, { team1: '', team2: '' }] : prev));
  };
  ...
  const derivedSetsWon = useMemo(() => {
    ...
    let t2 = 0;
    for (const s of sets) {
      // Ignora set con valori vuoti
      if (s.team1 === '' || s.team2 === '' || s.team1 == null || s.team2 == null) continue;
      if (s.team1 === s.team2) continue; // pari non conta (non valido)
      if (s.team1 > s.team2) t1 += 1;
      else t2 += 1;
    }
  ...
  const handleSubmit = async () => {
    // Validazione base set per set - filledSets esclude set con valori vuoti
    ...
    if (bestOf === 1) {
      ...
      const valid =
        filledSets.length === 1 &&
        ((score.team1 === 1 && score.team2 === 0) || (score.team1 === 0 && score.team2 === 1));
      if (!valid) {
      ...
      const valid =
        (score.team1 === 2 &&
          (score.team2 === 0 || score.team2 === 1) &&
          filledSets.length >= 2 &&
          filledSets.length <= 3) ||
        (score.team2 === 2 &&
          (score.team1 === 0 || score.team1 === 1) &&
          filledSets.length >= 2 &&
          filledSets.length <= 3);
      if (!valid) {
  ...
  const validation = useMemo(() => {
    ...
    const hasTieSet = filledSets.some(
      (s) => s.team1 === s.team2 && (s.team1 !== 0 || s.team2 !== 0)
    );
    ...
    if (bestOf === 1) {
      const ok =
        filledSets.length === 1 &&
        ((derivedSetsWon.team1 === 1 && derivedSetsWon.team2 === 0) ||
          (derivedSetsWon.team2 === 1 && derivedSetsWon.team1 === 0));
      return ok
        ? { valid: true }
        : { valid: false, reason: 'Inserisci esattamente 1 set con un vincitore (1-0 o 0-1)' };
    }
    if (bestOf === 3) {
      const ok =
        filledSets.length >= 2 &&
        filledSets.length <= 3 &&
        (derivedSetsWon.team1 === 2 || derivedSetsWon.team2 === 2);
      return ok
        ? { valid: true }
        : { valid: false, reason: 'Inserisci 2 o 3 set; vince chi arriva a 2 set' };
    }
  ...
  return createPortal(
    ...
    >
      ...
      <div className="flex min-h-screen items-center justify-center p-2 sm:p-4">
        {/* Modal content */}
        <div
          className={`relative ${T.card} rounded-xl shadow-2xl w-full max-w-2xl my-4 sm:my-8`}
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header */}
          <div className="flex items-center justify-between p-4 sm:p-6 border-b ring-0">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gray-700 rounded-lg flex items-center justify-center flex-shrink-0">
                <Trophy className={`w-4 h-4 sm:w-5 sm:h-5 ${T.accentInfo}`} />
              </div>
              <div className="min-w-0">
                <h2 id="modal-title" className={`text-lg sm:text-xl font-bold ${T.text} truncate`}>
                  {match?.status === 'completed' ? 'Modifica Risultato' : 'Inserisci Risultato'}
                </h2>
                <div className={`text-xs sm:text-sm ${T.subtext} mt-0.5 truncate`}>
            ...
            <button
              onClick={onClose}
              className={`${T.btnGhostSm} !p-1.5 sm:!p-2 flex-shrink-0`}
          ...
          <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 max-h-[calc(100vh-250px)] sm:max-h-[calc(100vh-200px)] overflow-y-auto">
            ...
            <div className="space-y-3 sm:space-y-4">
              {sets.map((s, idx) => {
                ...
                const invalidTennis = tennisRules
                  ? !validateTennisSet(s, idx, bestOf).valid
                  : false;

                return (
                  <div
                    key={idx}
                    className={`p-3 sm:p-4 rounded-lg ${setWinner ? '' : T.border} ${!setWinner ? 'bg-gray-800/50' : ''} ${
                      setWinner === 1
                  ...
                  >
                    <div className="flex items-center justify-between mb-2 sm:mb-3">
                      <div className={`text-sm sm:text-base font-semibold ${T.text}`}>
                        Set {idx + 1}
                      </div>
                      {setWinner && (
                        <div
                    ...
                    <div className="grid grid-cols-[1fr_auto_1fr] gap-2 sm:gap-3 items-center">
                      ...
                      <div className="space-y-1 sm:space-y-2">
                        <label
                          className={`block text-sm sm:text-base font-semibold ${setWinner === 1 ? 'text-emerald-400' : T.subtext} truncate`}
                        >
                          {team1Name}
                        </label>
                        <input
                          ...
                          min={0}
                          max={99}
                          value={typeof s.team1 === 'number' ? s.team1 : ''}
                          placeholder=""
                          onChange={(e) => updateSetScore(idx, 'team1', e.target.value)}
                          className={`${T.input} w-full text-center font-bold text-base sm:text-lg ${
                            setWinner === 1
                      ...
                      {/* Separator */}
                      <div className="text-xl sm:text-2xl font-bold text-gray-500 pb-6 sm:pb-8">
                        −
                      </div>
                      ...
                      <div className="space-y-1 sm:space-y-2">
                        <label
                          className={`block text-sm sm:text-base font-semibold ${setWinner === 2 ? 'text-blue-400' : T.subtext} truncate`}
                        >
                          {team2Name}
                        </label>
                        <input
                          ...
                          min={0}
                          max={99}
                          value={typeof s.team2 === 'number' ? s.team2 : ''}
                    ...
                    {!isTie && invalidTennis && (
                      <div className="mt-3 p-2 bg-amber-500/10 border border-amber-500/30 rounded-lg">
                        <p className="text-xs font-medium text-amber-400">
                          ⚠️ Punteggio non valido: minimo 6 con 2 giochi di scarto (7-5/7-6
                          ammessi). Nel terzo set è ammesso super tie-break a 10.
                        </p>
                      </div>
                    )}
                  </div>
            ...
            {winner && (
              <div className="bg-gradient-to-r from-emerald-500/10 to-emerald-600/10 border-2 border-emerald-500/30 rounded-xl p-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-emerald-400">Vincitore</p>
                    <p className="text-xl font-bold text-emerald-300 mt-0.5">{winner.name}</p>
            ...
            {!validation.valid && derivedSetsWon.team1 > 0 && (
              <div className="bg-amber-500/10 border-2 border-amber-500/30 rounded-xl p-4">
                <p className={`text-sm font-medium ${T.accentWarning}`}>⚠️ {validation.reason}</p>
              </div>
            )}
          </div>

          {/* Footer */}
          <div className="flex flex-col-reverse sm:flex-row items-center justify-end gap-2 sm:gap-3 p-4 sm:p-6 border-t ring-0">
            ...
            <button
              ...
              className={`${T.btnPrimary} w-full sm:w-auto ${!validation.valid || submitting ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              {submitting ? 'Salvataggio...' : `Conferma${bestOf === 1 ? ' (1 set)' : ' (2 su 3)'}`}
            </button>
          </div>
        </div>
      </div>
