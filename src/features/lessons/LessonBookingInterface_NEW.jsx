...
export default function LessonBookingInterface({ T, user: propUser, state, setState, clubMode }) {
  ...
  const findAvailableCourt = useCallback((date, time) => {
  ...
  const handleCreateLessonBooking = useCallback(async () => {
    if (!selectedInstructor || !selectedDate || !selectedTime) {
      setMessage({ type: 'error', text: 'Seleziona istruttore, data e ora per continuare.' });
      return;
    }
    ...
    try {
      ...
      if (!assignedCourt) {
        setMessage({ type: 'error', text: 'Nessun campo disponibile per questo orario.' });
        return;
      }
  ...
  }, [selectedInstructor, selectedDate, selectedTime, instructors, findAvailableCourt, createLessonBooking, actualUser, refreshLessons]);
            ...
            <div className="space-y-4">
              ...
              {instructors.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  Nessun istruttore disponibile. Contatta l'amministrazione per configurare gli istruttori.
                </div>
              ) : (
