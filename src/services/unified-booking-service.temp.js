...
export async function deleteBooking(bookingId, user) {
  console.log('🗑️ DELETE BOOKING called:', bookingId, 'user:', !!user, 'cloudStorage:', useCloudStorage);
...
function loadLocalBookings() {
  try {
    ...
    const activeBookings = allBookings.filter(booking => {
      ...
      const isActive = status !== BOOKING_STATUS.CANCELLED;
      if (!isActive) {
        console.log(`🗑️ Filtering out cancelled booking from localStorage:`, booking.id, booking.status);
      }
      return isActive;
    });
...
async function _getActiveUserBookings(userId) {
  ...
  if (!useCloudStorage) {
    ...
    const bookings = await getUserBookings(pseudoUser, { includeHistory: true, includeCancelled: false });
    return bookings.filter(b => b.date >= today && (b.status || BOOKING_STATUS.CONFIRMED) === BOOKING_STATUS.CONFIRMED);
  }
  try {
    const qActive = query(
      collection(db, COLLECTIONS.BOOKINGS),
    ...
    const pseudoUser = { uid: userId };
    return getUserBookings(pseudoUser, { includeHistory: true, includeCancelled: false });
  }
}
...
async function _getUserBookingHistory(userId) {
  ...
  if (!useCloudStorage) {
    const pseudoUser = { uid: userId };
    const bookings = await getUserBookings(pseudoUser, { includeHistory: true, includeCancelled: true });
    return bookings.filter(b => b.date < today).sort((a,b)=> b.date.localeCompare(a.date) || b.time.localeCompare(a.time));
  }
  try {
...
async function _searchBookingsForPlayer({ userId, email, name }) {
  ...
  if (!useCloudStorage) {
    ...
    return data.filter(b => {
      ...
      if (name) {
        ...
        if (Array.isArray(b.players) && b.players.some(p => (typeof p === 'string' ? p : p?.name) === name)) return true;
      }
      return false;
    });
  }
  try {
    const promises = [];
    if (userId) {
      promises.push(getDocs(query(collection(db, COLLECTIONS.BOOKINGS), where('createdBy','==', userId))));
    }
    if (email) {
      promises.push(getDocs(query(collection(db, COLLECTIONS.BOOKINGS), where('userEmail','==', email))));
    }
    if (name) {
      promises.push(getDocs(query(collection(db, COLLECTIONS.BOOKINGS), where('bookedBy','==', name))));
      // array-contains funziona solo se players è array di stringhe coerenti
      promises.push(getDocs(query(collection(db, COLLECTIONS.BOOKINGS), where('players','array-contains', name))));
    }
  ...
  if (nextBooking) {
    ...
    if (holeSize > 0 && holeSize < 30) {
      ...
      if (prevBooking) {
        ...
        if (totalGap === 120) {
          console.log(`🎯 EXEMPTION applied: exactly ${totalGap}min trapped slot allows ${holeSize}min hole`);
          return false;
        }
      }
...
export function isTimeSlotTrapped(courtId, date, startTime, duration, existingBookings) {
  ...
  const nextBooking = courtBookings
    .find(b => timeToMinutes(b.time) >= bookingEnd);
