...
const Toast = React.memo(function Toast({ 
  ...
  const isTestEnv =
    ...
    (typeof globalThis !== 'undefined' && globalThis.process && globalThis.process.env && globalThis.process.env.NODE_ENV === 'test') ||
  ...
  const handleClose = () => {
    ...
    if (isTestEnv) {
  try { onClose(id); } catch { /* ignore */ }
      // Some legacy tests expect onClose to be callable with no args
  try { if (onClose && onClose.length === 0) onClose(); } catch { /* ignore */ }
      return;
    }
    setTimeout(() => {
  try { onClose(id); } catch { /* ignore */ }
    ...
    >
      ...
      >
        {visibleToasts.map((toast) => (
          <div key={toast.id} className={`pointer-events-auto ${isTest ? 'fixed top-4 right-4 z-50' : ''}`}>
            <Toast
              id={toast.id}
              type={toast.type}
...
export function useToast(options = {}) {
  ...
  const isDuplicate = useCallback((message) => {
    ...
    return toasts.some(
      (toast) => 
        ...
        now - toast.createdAt < deduplicateTime
    );
  }, [toasts, deduplicateTime]);

  // Show toast (with queue + rate limit + deduplication)
  const showToast = useCallback((type, message, duration = 5000, extraOptions = {}) => {
    // Deduplicate check
  ...
  const showSuccess = useCallback((message, duration, options) => 
    showToast(TOAST_TYPES.SUCCESS, message, duration, options), [showToast]);

  const showError = useCallback((message, duration, options) => 
    showToast(TOAST_TYPES.ERROR, message, duration, options), [showToast]);

  const showWarning = useCallback((message, duration, options) => 
    showToast(TOAST_TYPES.WARNING, message, duration, options), [showToast]);

  const showInfo = useCallback((message, duration, options) => 
    showToast(TOAST_TYPES.INFO, message, duration, options), [showToast]);

  const showLoading = useCallback((message, options) => 
    showToast(TOAST_TYPES.LOADING, message, 0, options), [showToast]);
