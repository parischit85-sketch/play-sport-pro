    ...
    <div key={groupName} className="mb-6 last:mb-0">
        ...
        <table className="min-w-full bg-gray-800 border border-gray-700 rounded-lg">
          <thead className="bg-gray-700">
            <tr>
              ...
              <th className="px-4 py-3 text-center text-xs font-medium text-gray-400 uppercase tracking-wider font-bold">
                Punti
              </th>
            </tr>
          </thead>
            ...
            {teams.map((team, index) => (
              <tr
                ...
                className={`hover:bg-gray-700 transition-colors ${
                  team.qualified ? 'bg-green-900/20' : ''
                }`}
              >
                ...
                <td className="px-4 py-3 text-center whitespace-nowrap">
                  <span className="text-sm text-white">{team.played || 0}</span>
                </td>
                <td className="px-4 py-3 text-center whitespace-nowrap">
                  <span className="text-sm text-white">{team.won || 0}</span>
                </td>
                <td className="px-4 py-3 text-center whitespace-nowrap">
                  <span className="text-sm text-white">{team.lost || 0}</span>
                </td>
                <td className="px-4 py-3 text-center whitespace-nowrap">
                  <span className="text-sm text-white">{team.setsWon || 0}</span>
                </td>
                <td className="px-4 py-3 text-center whitespace-nowrap">
                  <span className="text-sm text-white">{team.setsLost || 0}</span>
                </td>
                <td className="px-4 py-3 text-center whitespace-nowrap">
                  <span
                ...
                <td className="px-4 py-3 text-center whitespace-nowrap">
                  <span className="text-sm font-bold text-blue-400">{team.points || 0}</span>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {teams.some((t) => t.qualified) && (
        <div className="mt-2 flex items-center text-xs text-gray-400">
          <span className="inline-block w-3 h-3 bg-green-900/20 border border-green-800 rounded mr-2"></span>
    ...
    <div className="bg-gray-800 rounded-lg shadow p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          {isLive && (
            <div className="flex items-center gap-2 px-2 py-1 bg-green-900/30 border border-green-700 rounded-full">
