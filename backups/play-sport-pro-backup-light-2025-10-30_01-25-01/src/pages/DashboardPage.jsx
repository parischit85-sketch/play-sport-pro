...
const QuickAction = React.memo(({ action, T: _T }) => (
  <button
    ...
    className={`relative bg-gray-800/70 backdrop-blur-xl border border-gray-600/40 hover:border-gray-500/60 p-6 rounded-2xl transition-all duration-300 hover:shadow-xl hover:-translate-y-1 group text-center overflow-hidden`}
  >
    {/* Subtle gradient overlay */}
    <div className="absolute inset-0 bg-gradient-to-br from-gray-700/30 via-transparent to-transparent pointer-events-none" />

    <div className="relative">
      <div
        className={`${action.iconWrap} w-12 h-12 backdrop-blur-sm border border-gray-600/20 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform mx-auto rounded-xl shadow-lg`}
      >
        {action.icon}
      </div>
    ...
    () => [
      {
        ...
        iconWrap:
          'bg-gradient-to-r from-blue-900/40 to-cyan-900/30 text-blue-400 border-blue-500/30',
      },
      {
        ...
        iconWrap:
          'bg-gradient-to-r from-emerald-900/40 to-green-900/30 text-emerald-400 border-emerald-500/30',
      },
    ],
    [navigate]
  );
        ...
        <div className="animate-pulse space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-gray-800/60 backdrop-blur-xl rounded-2xl h-24 border border-gray-700/20 shadow-xl"
              ></div>
            ))}
          </div>
        </div>
    ...
    <div className="space-y-1 min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-gray-800 overflow-x-hidden">
      ...
      {isInstructor ? (
        <div className="p-2">
          <React.Suspense
            fallback={
              <div className="bg-gray-800/80 backdrop-blur-xl rounded-3xl border border-gray-700/20 p-6 shadow-2xl animate-pulse">
        ...
        <>
          ...
          <div className="hidden lg:grid lg:grid-cols-2 lg:gap-6 lg:items-start lg:p-2">
            ...
            <div>
              <React.Suspense
                fallback={
                  <div className="bg-gradient-to-r from-gray-700/60 to-gray-800/40 rounded-2xl p-6 animate-pulse backdrop-blur-sm border border-gray-600/30">
                  ...
                  <div className="bg-gray-800/80 backdrop-blur-xl rounded-3xl border border-gray-700/20 p-6 shadow-2xl animate-pulse">
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-2">
                      {[1, 2].map((i) => (
                        <div
                          key={i}
                          className="min-w-[280px] h-32 bg-gray-600/40 rounded-2xl flex-shrink-0"
                        ></div>
                      ))}
                    </div>
                  </div>
          ...
          <div className="lg:hidden space-y-1 p-1">
            ...
            <div>
              <React.Suspense
                fallback={
                  <div className="bg-gradient-to-r from-gray-700/60 to-gray-800/40 rounded-2xl p-4 animate-pulse backdrop-blur-sm border border-gray-600/30">
                    <div className="h-3 bg-gray-600/60 rounded w-24 mb-3"></div>
                    <div className="flex gap-2 overflow-x-auto pb-2">
                      {[1, 2, 3].map((i) => (
                        <div
                          key={i}
                          className="min-w-[200px] h-24 bg-gray-600/40 rounded-lg flex-shrink-0"
                        ></div>
                      ))}
                    </div>
                  </div>
            ...
            <div>
              <React.Suspense
                fallback={
                  <div className="bg-gray-800/80 backdrop-blur-xl rounded-3xl border border-gray-700/20 p-4 shadow-2xl animate-pulse">
                    <div className="h-3 bg-gray-600/60 rounded w-24 mb-3"></div>
                    <div className="flex gap-2 overflow-x-auto pb-2">
                      {[1, 2].map((i) => (
                        <div
                          key={i}
                          className="min-w-[240px] h-28 bg-gray-600/40 rounded-2xl flex-shrink-0"
                        ></div>
                      ))}
                    </div>
                  </div>
