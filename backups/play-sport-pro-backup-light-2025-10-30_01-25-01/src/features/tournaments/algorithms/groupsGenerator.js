...
export async function generateBalancedGroups(
  ...
  try {
    ...
    const teamsWithRanking = teams
      .filter((t) => t.averageRanking !== null)
      .sort((a, b) => a.averageRanking - b.averageRanking); // Lower ranking = better
    ...
    groups.forEach((group) => {
      group.teams.forEach((team, index) => {
        assignmentPromises.push(
          assignTeamToGroup(clubId, tournamentId, team.id, group.id, index + 1)
        );
      });
    });
...
export function calculateGroupBalanceScore(groups) {
  ...
  const variance =
    avgRankings.reduce((sum, r) => sum + Math.pow(r - mean, 2), 0) / avgRankings.length;
