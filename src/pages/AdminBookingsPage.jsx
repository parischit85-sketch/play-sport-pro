    ...
    <div className="relative">
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-40" onClick={() => setShowSettings(false)}>
          <div 
            className="fixed inset-y-0 right-0 w-full max-w-5xl bg-white dark:bg-gray-900 shadow-xl overflow-y-auto z-50"
              ...
              <Extra
                ...
                setState={async (newState) => {
                  ...
                  if (newState.bookingConfig) {
                    try {
                      await updateBookingConfig(newState.bookingConfig, { source: 'admin-settings' });
                    } catch (error) {
                  ...
                  if (newState.courts && JSON.stringify(newState.courts) !== JSON.stringify(localCourts)) {
                    ...
                    const updatedCourts = newState.courts.filter(newCourt => {
                      ...
                      return existingCourt && JSON.stringify(existingCourt) !== JSON.stringify(newCourt);
                    });
                    ...
                    for (let i = 0; i < newCourts.length; i++) {
                      ...
                      try {
                        ...
                        const courtIndex = updatedNewState.courts.findIndex(c => c.id === court.id);
                        if (courtIndex !== -1) {
                    ...
                    for (const court of finalUpdatedCourts) {
                      try {
                        const courtRef = doc(db, 'clubs', clubId, 'courts', court.firebaseId || court.id);
