...
export function CacheMonitor({ isOpen, onClose, T }) {
  ...
  const handleClearCache = async (type) => {
    const confirmMessage = type === 'all' 
      ? 'Vuoi cancellare tutta la cache?' 
      : `Vuoi cancellare la cache di ${type}?`;
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 rounded-lg p-4 border border-green-200 dark:border-green-700">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold text-green-900 dark:text-green-300">Hit Rate</h3>
                ...
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-lg p-4 border border-blue-200 dark:border-blue-700">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold text-blue-900 dark:text-blue-300">Memory</h3>
                    <span className="text-2xl">⚡</span>
                  </div>
                ...
                <div className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-lg p-4 border border-purple-200 dark:border-purple-700">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold text-purple-900 dark:text-purple-300">IndexedDB</h3>
                    <span className="text-2xl">💿</span>
                  </div>
                ...
                <div className="bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 rounded-lg p-4 border border-orange-200 dark:border-orange-700">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold text-orange-900 dark:text-orange-300">Operations</h3>
                    <span className="text-2xl">🔄</span>
                  </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 mb-6">
                <h3 className="font-bold text-gray-900 dark:text-white mb-3">📊 Cache Performance</h3>
                <div className="space-y-3">
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      <span className={T.text}>Hit Rate</span>
                      <span className={parseFloat(stats.hitRate) > 70 ? 'text-green-600' : parseFloat(stats.hitRate) > 40 ? 'text-yellow-600' : 'text-red-600'}>
                        {stats.hitRate}%
                      </span>
                    </div>
                    <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                      <div 
                        className={`h-full transition-all ${
                          parseFloat(stats.hitRate) > 70 ? 'bg-green-500' : 
                          parseFloat(stats.hitRate) > 40 ? 'bg-yellow-500' : 
                          'bg-red-500'
                        }`}
                  ...
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className="text-blue-600">
                        {stats.memoryHits} / {stats.hits} ({stats.hits > 0 ? ((stats.memoryHits / stats.hits) * 100).toFixed(1) : 0}%)
                      </span>
                    </div>
                    <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-blue-500 transition-all"
                        style={{ width: `${stats.hits > 0 ? (stats.memoryHits / stats.hits) * 100 : 0}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className="text-purple-600">
                        {stats.indexedDBHits} / {stats.hits} ({stats.hits > 0 ? ((stats.indexedDBHits / stats.hits) * 100).toFixed(1) : 0}%)
                      </span>
                    </div>
                    <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-purple-500 transition-all"
                        style={{ width: `${stats.hits > 0 ? (stats.indexedDBHits / stats.hits) * 100 : 0}%` }}
                      />
                    </div>
                  </div>
                </div>
              ...
              {stats.indexedDBCollections && Object.keys(stats.indexedDBCollections).length > 0 && (
                <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                  <h3 className="font-bold text-gray-900 dark:text-white mb-3">📦 Cached Collections</h3>
                  <div className="space-y-2">
                    {Object.entries(stats.indexedDBCollections).map(([collection, data]) => (
                      <div key={collection} className="flex items-center justify-between p-3 bg-white dark:bg-gray-800 rounded-lg">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-red-100 hover:bg-red-200 dark:bg-red-900/30 dark:hover:bg-red-900/50 text-red-700 dark:text-red-300 rounded text-sm font-medium transition-colors"
                        >
                          Clear
                        </button>
                      </div>
              ...
              <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-700">
                <h4 className="font-semibold text-blue-900 dark:text-blue-300 mb-2">💡 Optimization Tips</h4>
                <ul className="text-sm text-blue-800 dark:text-blue-400 space-y-1">
                  {parseFloat(stats.hitRate) < 70 && (
                    <li>• Low hit rate ({stats.hitRate}%) - Consider increasing cache TTL or warming critical data</li>
                  )}
                  {stats.indexedDBSize > 1000 && (
                    <li>• Large IndexedDB cache ({stats.indexedDBSize} items) - Consider periodic cleanup</li>
                  )}
                  {stats.memorySize < 20 && stats.hits > 100 && (
                    <li>• Low memory cache usage - Consider increasing memoryMaxSize for better performance</li>
                  )}
                  {parseFloat(stats.hitRate) >= 70 && (
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
          <div className="flex justify-between items-center">
            <div className="text-xs text-gray-600 dark:text-gray-400">
              Last updated: {new Date().toLocaleTimeString()} {autoRefresh && '(Auto-refreshing every 3s)'}
            </div>
            <button
              onClick={onClose}
