...
const StatCard = ({ title, value, icon, color = 'blue', subtitle, trend }) => {
  const colors = {
    blue: 'bg-blue-50 bg-blue-900/20 text-blue-600 text-blue-400',
    green: 'bg-green-50 bg-green-900/20 text-green-600 text-green-400',
    purple: 'bg-purple-50 bg-purple-900/20 text-purple-600 text-purple-400',
    orange: 'bg-orange-50 bg-orange-900/20 text-orange-600 text-orange-400',
    red: 'bg-red-50 bg-red-900/20 text-red-600 text-red-400',
  };

  return (
    <div className="bg-white bg-gray-800 rounded-lg shadow p-6 hover:shadow-lg transition-shadow">
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <div className="ml-4">
            <p className="text-sm font-medium text-gray-600 text-gray-400">{title}</p>
...
const QuickActions = ({
  ...
  return (
    <div className="bg-white bg-gray-800 rounded-lg shadow p-6">
      <h2 className="text-lg font-semibold text-gray-900 text-white mb-4">Azioni Rapide</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
        ...
        <button
          ...
          className="flex items-center justify-center gap-3 border border-gray-300 border-gray-600 
                   text-gray-700 text-gray-300 px-4 py-3 rounded-lg font-medium
                   hover:bg-gray-50 hover:bg-gray-700 transition-colors"
        >
          <span>🏟️</span>
          Gestisci Club
        </button>
...
const MigrationWarningBanner = ({ onCreateSportingCAT, onMigrateData, clubs }) => {
  ...
  return (
    <div
      className="mb-6 bg-gradient-to-r from-amber-50 to-orange-50 from-amber-900/20 to-orange-900/20 
      ...
      <div className="flex justify-between items-start">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="text-lg font-semibold text-amber-800 text-amber-200">
              Migrazione Dati Richiesta
            </h3>
          </div>

          <div className="text-amber-700 text-amber-300 space-y-2">
        ...
        <button
          ...
          className="text-amber-600 hover:text-amber-800 text-amber-400 hover:text-amber-200 
  ...
  return (
    <div className="bg-white bg-gray-800 rounded-lg shadow p-6">
      ...
      <div className="space-y-4">
        {activities.length === 0 ? (
          <p className="text-sm text-gray-500 text-gray-400 text-center py-8">
          ...
          activities.map((activity, index) => (
            <div
              key={index}
              className="flex items-start gap-3 hover:bg-gray-50 hover:bg-gray-700 p-2 rounded-lg transition-colors"
            >
              <span className="text-xl flex-shrink-0">{activity.icon}</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm text-gray-900 text-white truncate">{activity.text}</p>
      ...
      <div className="space-y-3">
        {affiliations.length === 0 ? (
          ...
          affiliations.slice(0, 5).map((affiliation) => (
            <div
              key={affiliation.id}
              className="border border-gray-200 border-gray-600 rounded-lg p-4"
            >
              <div className="flex justify-between items-start mb-2">
                <div className="flex-1">
                  ...
                  {affiliation.notes && (
                    <p className="text-xs text-gray-600 text-gray-300 mt-1 italic">
                      "{affiliation.notes}"
                    </p>
                  )}
                </div>
                <div className="flex gap-2 ml-4">
                  <button
                    ...
                    className="text-green-600 hover:text-green-800 text-green-400 hover:text-green-300"
                    title="Approva"
                  >
                    ✅
                  </button>
        ...
        {affiliations.length > 5 && (
          <div className="text-center pt-2">
            <button className="text-sm text-blue-600 hover:text-blue-800 text-blue-400 hover:text-blue-300">
...
const ClubCreateModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-90vh overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-semibold text-gray-900 text-white">Crea Nuovo Club</h2>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-4">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 border border-gray-300 border-gray-600 
                         text-gray-700 text-gray-300 rounded-lg font-medium
                         hover:bg-gray-50 hover:bg-gray-700 transition-colors"
              >
                Annulla
              </button>
...
const SystemStatus = ({ stats, loading }) => {
  if (loading) {
    return (
      <div className="bg-white bg-gray-800 rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-gray-900 text-white mb-4">Stato Sistema</h2>
        <div className="flex justify-center py-8">
          <LoadingSpinner />
        </div>
      </div>
    ...
    <div className="bg-white bg-gray-800 rounded-lg shadow p-6">
      ...
      <div className="space-y-3">
        {healthMetrics.map((metric, index) => (
          <div
            key={index}
            className="flex justify-between items-center py-2 border-b border-gray-100 border-gray-700 last:border-b-0"
          >
            <div className="flex items-center gap-2">
              <span className="text-lg">{getStatusIcon(metric.status)}</span>
              <span className="text-sm text-gray-700 text-gray-300">{metric.label}</span>
            </div>
...
// --- Admin utility: Revert Championship Points for a Tournament ---
const AdminChampionshipRevertTool = () => {
    ...
    try {
      ...
      if (res.success) {
        if (res.alreadyReverted) {
          setMessage({
            type: 'info',
            text: 'Nessun punto da annullare (già annullato o non applicato).',
          });
        } else {
  ...
  return (
    <div className="bg-white bg-gray-800 rounded-lg shadow p-6">
      <h2 className="text-lg font-semibold text-gray-900 text-white mb-4">
        Annulla Punti Campionato (Admin)
      </h2>
      <form onSubmit={onRevert} className="space-y-3">
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 border-gray-600 rounded-lg bg-white bg-gray-700 text-gray-900 text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          />
        </div>
        <div>
          <label
