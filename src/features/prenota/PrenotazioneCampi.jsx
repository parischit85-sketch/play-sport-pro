...
export default function PrenotazioneCampi({ state, setState, players, playersById, T }) {
  ...
  const [mobileView, setMobileView] = useState('overview'); // 'overview' o 'detail'
  const [selectedCourtFilter, setSelectedCourtFilter] = useState('all'); // per vista dettaglio
  ...
  useEffect(() => {
    if (form.bookingType === 'lezione' && form.instructorId && instructors.length > 0) {
      ...
      if (selectedInstructor?.instructorData?.color) {
        setForm((f) => ({ ...f, color: selectedInstructor.instructorData.color }));
      }
    ...
    <Section title="Gestione Campi" T={T}>
      ...
      {isDesktop && (
        <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
          <div className="flex items-center gap-2 text-sm text-blue-700 dark:text-blue-300">
            ...
            <span>Trascina le prenotazioni per spostarle su slot liberi dello stesso giorno</span>
          </div>
        </div>
      )}
      ...
      {!state ? (
        ...
        <>
            ...
            <div className="flex bg-gray-100 dark:bg-gray-800 rounded-lg p-1 gap-1">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-md text-sm font-semibold transition-all ${
                  mobileView === 'overview'
                    ? 'bg-white dark:bg-gray-700 text-blue-600 dark:text-emerald-400 shadow-sm'
          ...
          {/* Vista Mobile - Panoramica Compatta */}
          {mobileView === 'overview' && (
            <div className="md:hidden mb-6">
              <div className="grid grid-cols-2 gap-3">
                {courts.map((court) => {
                  // Conta slot liberi per questo campo usando la logica di overlapping
                  const freeSlots = timeSlots.filter((time) => {
                    const slotStart = time;
                    const slotEnd = addMinutes(time, cfg.slotMinutes);
                    const inSchedule = isCourtBookableAt(time, court.id, courts);

                    const hasConflict = bookingsByCourt.get(court.id)?.some((b) => {
                      const bStart = new Date(b.start);
                      const bEnd = addMinutes(new Date(b.start), b.duration);
                      return overlaps(slotStart, slotEnd, bStart, bEnd);
                    });

                    return inSchedule && !hasConflict;
                  }).length;

                  const totalSlots =
                    timeSlots.filter((time) => isCourtBookableAt(time, court.id, courts)).length ||
                    0;
                  const occupiedSlots = totalSlots - freeSlots;
                  const availability = freeSlots / totalSlots;

                  return (
                    <div
                      key={court.id}
                      className={`${T.cardBg} ${T.border} rounded-xl p-4 shadow-md`}
                    >
                      <div className="flex items-center gap-2 mb-3">
                        <span
                          className={`w-8 h-8 rounded-full bg-blue-400 dark:bg-emerald-400 text-white flex items-center justify-center font-bold text-sm shadow`}
                        >
                          {court.name[0]}
                        </span>
                      ...
                      <button
                        ...
                        className={`w-full ${T.btnGhost} py-2 text-xs font-semibold`}
                      >
            ...
            <div className="md:hidden mb-6">
              ...
              <div className="mb-4">
                <select
                  ...
                  className={`w-full ${T.input} text-sm`}
                >
              ...
              <div className="space-y-2">
                {timeSlots.map((time) => {
                  ...
                  const endTime = addMinutes(time, cfg.slotMinutes);
                  const endTimeStr = endTime.toLocaleTimeString('it-IT', {
                    hour: '2-digit',
                    minute: '2-digit',
                  });
                  ...
                  return (
                    <div
                      key={time.getTime()}
                      className={`${T.cardBg} ${T.border} rounded-lg p-3 shadow-sm`}
                    >
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {courts
                          .filter(
                            (court) =>
                              selectedCourtFilter === 'all' || court.id === selectedCourtFilter
                          )
                          .map((court) => {
                            // Controlla se lo slot si sovrappone con una prenotazione esistente
                            const slotStart = time;
                            const slotEnd = addMinutes(time, cfg.slotMinutes);
                            const inSchedule = isCourtBookableAt(time, court.id, courts);

                            const existingBooking = bookingsByCourt.get(court.id)?.find((b) => {
                              const bStart = new Date(b.start);
                              const bEnd = addMinutes(new Date(b.start), b.duration);
                              return overlaps(slotStart, slotEnd, bStart, bEnd);
                            });
                            ...
                            if (basicAvailable) {
                              ...
                              const allCourtBookings = Array.from(
                                bookingsByCourt.get(court.id) || []
                              ).map((b) => ({
                                courtId: court.id,
                            ...
                            return (
                              <button
                                key={court.id}
                                onClick={() => {
                                  if (!inSchedule) return; // fuori fascia, niente azione
                                  if (isAvailable) return openCreate(court.id, time);
                                  if (existingBooking) return openEdit(existingBooking);
                                }}
                                className={`p-2 rounded-lg text-sm font-medium transition-all ${
                                  isAvailable
                                    ? `hover:scale-105 ${T.btnGhost} border-2 ${hasPromo ? 'border-yellow-400 bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20' : 'border-green-200 dark:border-green-700'}`
                                    : inSchedule
                                      ? 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border-2 border-red-200 dark:border-red-800 hover:bg-red-100 dark:hover:bg-red-900/30 cursor-pointer'
                                      : 'bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500 border-2 border-dashed border-gray-300 dark:border-gray-700 cursor-not-allowed'
                                }`}
                              >
                                ...
                                {inSchedule && !isAvailable && existingBooking && (
                                  <div className="text-xs mt-1 truncate">
                                    <div className="font-medium">
                                      {existingBooking.bookedByName || 'Occupato'}
                                    </div>
          ...
          {/* Griglia Desktop (nascosta su mobile) */}
