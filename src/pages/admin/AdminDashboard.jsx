  ...
  const onCheck = async (e) => {
    ...
    try {
      ...
      if (status.applied) {
        setMessage({
          type: 'info',
          text: `Punti applicati trovati. appliedAt: ${status.data?.appliedAt || 'n/d'}`,
        });
      } else {
        setMessage({
          type: 'error',
          text: 'Nessun audit trovato per questi ID (probabilmente già annullati o ID non corrispondenti).',
        });
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      ...
      <form onSubmit={onRevert} className="space-y-3">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="clubId">
            clubId
          </label>
          <input
            id="clubId"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="tournamentId">
            tournamentId
          </label>
          <input
            id="tournamentId"
