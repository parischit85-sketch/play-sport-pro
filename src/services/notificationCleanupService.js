...
class NotificationCleanupService {
  ...
  async cleanupOldScheduledNotifications() {
    ...
    try {
      ...
      console.log(`✅ Deleted ${deletedCount} old scheduled notifications (> ${this.retentionDays.scheduled} days)`);
      return deletedCount;
  ...
  async cleanupOldDeliveryLogs() {
    ...
    try {
      ...
      console.log(`✅ Deleted ${deletedCount} old delivery logs (> ${this.retentionDays.deliveries} days)`);
      return deletedCount;
  ...
  async cleanupOrphanedData() {
    ...
    try {
      ...
      for (const subDoc of subscriptionsSnapshot.docs) {
        ...
        const userDoc = await getDocs(query(usersRef, where('__name__', '==', subscription.userId), limit(1)));
  ...
  setRetentionDays(config) {
    ...
    if (config.expiredSubscriptions) this.retentionDays.expiredSubscriptions = config.expiredSubscriptions;
  }
