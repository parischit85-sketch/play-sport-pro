...
export default function ModernMultiLineChart({
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div
          className={`bg-white/95 bg-gray-800/95 backdrop-blur-sm border border-gray-200 border-gray-700 rounded-xl shadow-xl ${isMobile ? 'p-2 min-w-36' : 'p-4 min-w-48'}`}
        >
          <p
            className={`${isMobile ? 'text-xs' : 'text-sm'} font-semibold text-gray-900 text-gray-100 ${isMobile ? 'mb-1' : 'mb-3'}`}
          >
            {label}
          </p>
              ...
              .map((entry, index) => (
                <div key={index} className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    />
                    <span
                      className={`${isMobile ? 'text-xs' : 'text-sm'} font-medium text-gray-700 text-gray-300 ${isMobile ? 'truncate max-w-16' : ''}`}
                    >
          ...
          return (
            <div key={entry.value} className="flex items-center gap-1.5 md:gap-2">
              <div
                ...
                style={{ backgroundColor: entry.color }}
              />
              <span className="text-xs md:text-sm font-medium text-gray-700 text-gray-300 truncate max-w-20 md:max-w-none">
                {entry.value}
              </span>
              <span className="text-xs font-semibold text-gray-600 text-gray-400 bg-gray-100 bg-gray-700 px-1 md:px-1.5 py-0.5 rounded">
                {position}°
              </span>
            </div>
          );
  ...
  return (
    <div className="h-64 md:h-72 lg:h-80">
        ...
        >
          ...
          {seriesKeys.map((key, index) => (
            <Line
              ...
              strokeWidth={
                window.innerWidth < 768 ? (index < 3 ? 3 : 2) : index < 3 ? 4 : index < 5 ? 3 : 2
              }
              dot={{
                r:
                  window.innerWidth < 768 ? (index < 3 ? 3 : 2) : index < 3 ? 4 : index < 5 ? 3 : 2,
              ...
              activeDot={{
                r:
                  window.innerWidth < 768 ? (index < 3 ? 5 : 4) : index < 3 ? 7 : index < 5 ? 6 : 5,
                fill: PODIUM_COLORS[index] || `hsl(${index * 36}, 70%, 50%)`,
                stroke: 'white',
      ...
      <div className="flex justify-between items-center mt-2 md:mt-3 px-1 md:px-2">
        ...
        <span className="text-xs text-gray-500">{last10Data.length} periodi</span>
      </div>
            ...
            {seriesKeys.slice(0, 3).map((key, index) => {
              ...
              return (
                <div key={key} className="flex items-center gap-1 text-xs">
                  ...
                  />
                  <span className="font-medium text-gray-700 text-gray-300 truncate max-w-12">
                    {key.split(' ')[0]}
                  </span>
