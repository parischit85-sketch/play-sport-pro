              ...
              <div className="grid grid-cols-2 gap-3">
                {courts.map((court) => {
                  ...
                  const freeSlots = timeSlots.filter((time) => {
                    ...
                    const inSchedule = isCourtBookableAt(
                      time,
                      court.id,
                      courts,
                    );

                    const hasConflict = bookingsByCourt
                      .get(court.id)
                      ?.some((b) => {
                        const bStart = new Date(b.start);
                        const bEnd = addMinutes(new Date(b.start), b.duration);
                  ...
                  const totalSlots =
                    timeSlots.filter((time) =>
                      isCourtBookableAt(time, court.id, courts),
                    ).length || 0;
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {courts
                          ...
                          .map((court) => {
                            ...
                            const existingBooking = bookingsByCourt
                              .get(court.id)
                              ?.find((b) => {
                                const bStart = new Date(b.start);
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  if (isAvailable)
                                    return openCreate(court.id, time);
                                  if (existingBooking)
                                    return openEdit(existingBooking);
                                }}
