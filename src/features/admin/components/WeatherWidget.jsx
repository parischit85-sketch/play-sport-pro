...
const WeatherWidget = ({ club }) => {
    ...
    const fetchWeather = async () => {
      ...
      const response = await fetch(
        ...
        const getTimeSlotData = (hourlyData, dateStr, startHour, endHour) => {
          ...
          return {
            temp: temps.length > 0 ? Math.round(temps.reduce((a, b) => a + b, 0) / temps.length) : 0,
  ...
  if (loading) {
    return (
      <div className="bg-gradient-to-br from-blue-50 to-sky-50 dark:from-blue-900/20 dark:to-sky-900/20 rounded-xl border border-blue-200/50 dark:border-blue-700/50 p-4">
  ...
  if (error) {
    return (
      <div className="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800/50 dark:to-gray-700/50 rounded-xl border border-gray-200/50 dark:border-gray-600/50 p-4">
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 via-sky-50 to-cyan-50 dark:from-blue-900/20 dark:via-sky-900/20 dark:to-cyan-900/20 rounded-xl border border-blue-200/50 dark:border-blue-700/50 p-2 shadow-lg">
      ...
      {/* Oggi */}
      <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-lg p-1.5 border border-blue-100 dark:border-blue-700/30">
        <div className="flex items-center justify-between mb-1">
          <div className="text-xs font-bold text-blue-600 dark:text-blue-400">
            {formatDate(weather.today.date)}
          </div>
        ...
        <div className="grid grid-cols-3 gap-1">
          {/* Mattina */}
          <div 
            className="text-center cursor-pointer hover:bg-blue-50 dark:hover:bg-gray-700 rounded px-0.5 py-1 transition-all"
            onClick={() => handleDayClick(0)}
            onKeyDown={(e) => e.key === 'Enter' && handleDayClick(0)}
            role="button"
            tabIndex={0}
          >
            <div className="text-[10px] text-gray-600 dark:text-gray-400 leading-none mb-1">Mattina</div>
            <div className="text-3xl leading-none mb-1">{getWeatherEmoji(weather.today.morning.weatherCode)}</div>
            <div className="text-sm font-bold text-gray-800 dark:text-gray-200 leading-none mb-0.5">
              {weather.today.morning.temp}°
            </div>
            {weather.today.morning.precipProb > 0 && (
              <div className="text-[10px] text-blue-600 dark:text-blue-400 leading-none">
          ...
          <div 
            ...
            tabIndex={0}
          >
            <div className="text-[10px] text-gray-600 dark:text-gray-400 leading-none mb-1">Pomer.</div>
            <div className="text-3xl leading-none mb-1">{getWeatherEmoji(weather.today.afternoon.weatherCode)}</div>
            <div className="text-sm font-bold text-gray-800 dark:text-gray-200 leading-none mb-0.5">
          ...
          <div 
            ...
            tabIndex={0}
          >
            <div className="text-[10px] text-gray-600 dark:text-gray-400 leading-none mb-1">Sera</div>
            <div className="text-3xl leading-none mb-1">{getWeatherEmoji(weather.today.evening.weatherCode)}</div>
            <div className="text-sm font-bold text-gray-800 dark:text-gray-200 leading-none mb-0.5">
      ...
      <div className="mt-1.5 pt-1 border-t border-blue-100 dark:border-blue-700/30">
        <div className="text-[10px] text-gray-500 dark:text-gray-400 text-center leading-none">
          Clicca per dettagli
        </div>
      </div>
                ...
                <div className="space-y-2">
                  {hourlyForecast.map((item) => {
                    ...
                    return (
                      <div 
                        key={item.time}
                        className="bg-gradient-to-r from-blue-50 to-sky-50 dark:from-gray-700 dark:to-gray-600 rounded-lg p-3 flex items-center justify-between hover:shadow-md transition-shadow"
                      >
                        <div className="flex items-center gap-4 flex-1">
                          <div className="text-lg font-bold text-gray-700 dark:text-gray-200 w-16">
