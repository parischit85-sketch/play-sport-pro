...
class BackupService {
    ...
    try {
      ...
      for (const collectionName of collections) {
        ...
        try {
          ...
          await this.restoreCollection(collectionName, backupData.data[collectionName], overwrite);
  ...
  validateBackup(backup) {
    ...
    if (size > BACKUP_CONFIG.MAX_SIZE_MB) {
      throw new Error(`Backup too large: ${size}MB (max ${BACKUP_CONFIG.MAX_SIZE_MB}MB)`);
    }
  ...
  async getBackupHistory(limit = 20) {
    try {
      ...
      const q = query(backupHistoryRef, orderBy('createdAt', 'desc'), firestoreLimit(limit));
  ...
  async getRestoreHistory(limit = 20) {
    try {
      ...
      const q = query(restoreHistoryRef, orderBy('createdAt', 'desc'), firestoreLimit(limit));
