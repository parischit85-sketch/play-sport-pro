...
export class ReportGenerator {
    ...
    const stats = {
      ...
      avgDuration:
        bookings.length > 0
          ...
          : 0,
      avgPlayers:
        bookings.length > 0 ? bookings.reduce((sum, b) => sum + b.players, 0) / bookings.length : 0,
    };
  ...
  async getMatchesReport(clubId, startDate, endDate) {
    ...
    for (const doc of snapshot.docs) {
      ...
      matches.push({
        ...
        score: data.score ? `${data.score.player1} - ${data.score.player2}` : 'N/A',
        winner: data.winner || 'N/A',
        eloChange: data.eloChange ? `${data.eloChange.player1} / ${data.eloChange.player2}` : 'N/A',
        status: data.status,
      });
    }
  ...
  async getUsersReport(clubId, startDate, endDate) {
    ...
    const stats = {
      ...
      avgBookingsPerUser:
        users.length > 0 ? users.reduce((sum, u) => sum + u.bookings, 0) / users.length : 0,
      avgMatchesPerUser:
        users.length > 0 ? users.reduce((sum, u) => sum + u.matches, 0) / users.length : 0,
      ...
      switch (type) {
        case 'bookings':
          if (format === 'excel') {
            ...
            result = await reportGenerator.exportBookingsToPDF(
              clubId,
              clubName,
              startDate,
              endDate
