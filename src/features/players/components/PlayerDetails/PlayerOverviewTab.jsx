  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Status Card */}
        <div
          className={`${T.cardBg} ${T.border} rounded-2xl p-5 shadow-lg hover:shadow-xl transition-shadow`}
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-3xl">{player.isActive ? '✅' : '⏸️'}</span>
            <div
              className={`px-3 py-1 rounded-full text-xs font-bold ${
                player.isActive ? `bg-green-900/30 ${T.accentSuccess}` : `bg-gray-700 ${T.subtext}`
              }`}
            >
          ...
          <div className={`text-xs ${T.subtext}`}>Membro dal {formatDate(player.createdAt)}</div>
        </div>
        ...
        >
          ...
          <div className={`text-lg font-bold ${T.text}`}>{formatDate(player.lastActivity)}</div>
          <div className={`text-xs ${T.subtext} mt-1`}>
            {Math.floor(
              (new Date() - new Date(player.lastActivity || Date.now())) / (1000 * 60 * 60 * 24)
            )}{' '}
            giorni fa
          </div>
        </div>
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-3xl">🎾</span>
            <div
              className={`px-3 py-1 rounded-full text-xs font-bold bg-blue-900/30 ${T.accentInfo}`}
            >
          ...
          <div className="space-y-4">
            ...
            {player.dateOfBirth && (
              <div className="flex items-start gap-3 p-3 rounded-xl bg-gray-800/50 hover:bg-gray-800 transition-colors">
                <span className="text-xl flex-shrink-0">🎂</span>
                <div className="flex-1 min-w-0">
                  <label className={`block text-xs font-semibold ${T.subtext} mb-1`}>
                    Data di Nascita
                  </label>
                  <div className={`${T.text} text-sm font-medium`}>
                    ...
                    <span className={`ml-2 text-xs ${T.subtext}`}>
                      (
                      {Math.floor(
                        (new Date() - new Date(player.dateOfBirth)) / (1000 * 60 * 60 * 24 * 365)
                      )}{' '}
                      anni)
                    </span>
                  </div>
          ...
          {player.tournamentData?.isParticipant && player.tournamentData?.isActive ? (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {/* Initial Ranking */}
                <div
                  className={`bg-gradient-to-br from-orange-900/20 to-orange-800/20 rounded-xl p-4 border-orange-700 ${T.border}`}
                >
                ...
                {/* Current Ranking */}
                <div
                  className={`bg-gradient-to-br from-purple-900/20 to-purple-800/20 rounded-xl p-4 border-purple-700 ${T.border}`}
                >
                  <div className={`text-xs font-semibold ${T.subtext} mb-2`}>
                    🏆 Ranking Attuale
                  </div>
            ...
            <div className="text-center py-8 px-4 bg-gradient-to-br from-gray-50 to-gray-100 from-gray-800/50 to-gray-900/50 rounded-xl border-2 border-dashed border-gray-700">
              ...
              <span className={`text-sm font-semibold ${T.text}`}>Non partecipa al campionato</span>
          ...
          <div>
            ...
            {player.tags && player.tags.length > 0 ? (
              ...
              <div className="p-6 text-center bg-gray-700/50 rounded-xl border-2 border-dashed border-gray-700">
                ...
                <div className={`text-sm ${T.subtext} italic`}>Nessun tag assegnato</div>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            {player.playingPreferences && player.playingPreferences.length > 0 ? (
              ...
              <div className="p-6 text-center bg-gray-700/50 rounded-xl border-2 border-dashed border-gray-700">
                ...
                <div className={`text-sm ${T.subtext} italic`}>Nessuna preferenza impostata</div>
              </div>
            )}
          </div>
        </div>
