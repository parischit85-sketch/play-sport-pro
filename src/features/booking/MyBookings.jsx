...
export default function MyBookings({ bookings, user, onCancel, T }) {
  ...
  const userBookings = bookings.filter(
    (booking) => booking.bookedBy === (user?.displayName || user?.email)
  );
          ...
          {sortedBookings.map((booking) => (
            ...
            >
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-baseline gap-1 mb-2">
                    <span className={`${ds.h4} font-bold`}>{booking.time.substring(0, 5)}</span>
              ...
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-2">
                      ...
                      booking.players.slice(0, 4).map((player, index) => (
                        <div
                          ...
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold text-white border-2 border-white ${
                            index === 0
                              ? 'bg-blue-500'
                              : ['bg-green-500', 'bg-purple-500', 'bg-orange-500'][index - 1] ||
                                'bg-gray-500'
                          }`}
                          title={player}
                        >
                  ...
                  {!booking.players || booking.players.length < 4 ? (
                    ...
                    <span className={`${ds.bodySm} text-green-600 text-green-400 ml-1`}>
                      Al completo
                    </span>
                  )}
                </div>
              ...
              <div className="flex gap-2 mb-4">
                {booking.lighting && (
                  <div className="flex items-center gap-1 px-2 py-1 bg-yellow-100 bg-yellow-900/20 rounded-full">
                    <svg
                    ...
                    <span className={`${ds.bodySm} text-yellow-800 text-yellow-200`}>
                      Illuminazione
                    </span>
                  </div>
                )}
                {booking.heating && (
                  <div className="flex items-center gap-1 px-2 py-1 bg-red-100 bg-red-900/20 rounded-full">
              ...
              <div className="flex items-center justify-between pt-3 border-t border-gray-200 border-gray-700">
                <div className="flex flex-col">
                  <span className={`${ds.bodySm} text-gray-500`}>Totale</span>
                  <span className={`${ds.h5} font-bold text-green-600 text-green-400`}>
                    €{booking.price}
                  </span>
                </div>
                ...
                {isUpcoming(booking.date, booking.time) && (
                  <div className="flex gap-2">
                    <button
                      className={`${T.btnSm} bg-blue-50 bg-blue-900/20 text-blue-600 text-blue-400 hover:bg-blue-100 hover:bg-blue-900/40 border-0`}
                    >
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    {canCancel(booking.date, booking.time) && (
                      <button
                        ...
                        className={`${T.btnSm} bg-red-50 bg-red-900/20 text-red-600 text-red-400 hover:bg-red-100 hover:bg-red-900/40 border-0`}
                      >
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                          <path
                            fillRule="evenodd"
      ...
      <div className="flex justify-center mt-4 md:hidden">
        <div className="flex gap-1">
          {sortedBookings.slice(0, 5).map((_, index) => (
            <div key={index} className="w-2 h-2 rounded-full bg-gray-300 bg-gray-600"></div>
          ))}
          {sortedBookings.length > 5 && <div className="w-2 h-2 rounded-full bg-blue-500"></div>}
        </div>
      </div>
    </Section>
  );
