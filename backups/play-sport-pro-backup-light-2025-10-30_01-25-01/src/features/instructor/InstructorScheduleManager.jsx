...
export default function InstructorScheduleManager({ compact = false }) {
  ...
  if (loading) {
    return (
      <div
        className={`bg-white/80 bg-gray-800/80 backdrop-blur-xl rounded-3xl border border-white/20 border-gray-700/20 p-6 shadow-2xl ${compact ? 'mb-4' : ''}`}
      >
        <div className="animate-pulse space-y-4">
          <div className="h-6 bg-gray-200/60 bg-gray-600/40 rounded w-48"></div>
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-16 bg-gray-200/40 bg-gray-600/30 rounded-lg"></div>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {slotsForSelectedDate.length === 0 ? (
          ...
          slotsForSelectedDate.map((slot) => (
            <div
              key={slot.id}
              className="bg-gradient-to-r from-gray-50/80 to-gray-100/60 from-gray-700/60 to-gray-800/40 backdrop-blur-sm rounded-2xl border border-white/30 border-gray-600/30 p-4"
            >
              {/* Slot header */}
              <div className="flex items-center justify-between mb-3">
                <div className="text-lg font-semibold text-gray-900 text-white">
              ...
              <div className="space-y-2 text-sm">
                ...
                {slot.notes && (
                  <div className="mt-3 p-3 bg-blue-50/70 bg-blue-900/20 backdrop-blur-sm rounded-lg border border-blue-200/50 border-blue-700/30">
                    <div className="text-xs text-blue-700 text-blue-300 font-medium mb-1">
                      Note:
                    </div>
                    <div className="text-sm text-blue-800 text-blue-200">{slot.notes}</div>
                  </div>
                )}
              </div>
            </div>
        ...
        <form onSubmit={handleSubmitSlot} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-300 border-gray-600 rounded-lg bg-white bg-gray-700 text-gray-900 text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              required
            >
