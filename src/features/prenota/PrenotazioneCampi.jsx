        ...
        <div className="md:hidden mb-6">
          <div className="grid grid-cols-2 gap-3">
            {courts.map((court) => {
              ...
              return (
                <div key={court.id} className={`${T.cardBg} ${T.border} rounded-xl p-4 shadow-md`}>
                  <div className="flex items-center gap-2 mb-3">
                    <span className={`w-8 h-8 rounded-full bg-blue-400 dark:bg-emerald-400 text-white flex items-center justify-center font-bold text-sm shadow`}>
                      {court.name[0]}
                    </span>
                    <div className="flex-1">
                      <div className="font-semibold text-sm">{court.name}</div>
                      {court.hasHeating && (
                        <div className="text-xs text-orange-500 dark:text-orange-400">🔥 Riscaldato</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mb-2">
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-green-600 dark:text-green-400">Liberi: {freeSlots}</span>
                      <span className="text-red-600 dark:text-red-400">Occupati: {occupiedSlots}</span>
                    </div>
                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                      <div 
                        className={`h-2 rounded-full transition-all duration-300 ${
                          availability > 0.7 ? 'bg-green-500' :
                          availability > 0.4 ? 'bg-yellow-500' : 'bg-red-500'
                        }`}
        ...
        <div className="md:hidden mb-6">
          ...
          <div className="space-y-2">
            {timeSlots.map((time) => {
              ...
              return (
                <div key={time.getTime()} className={`${T.cardBg} ${T.border} rounded-lg p-3 shadow-sm`}>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {courts
                      .filter(court => selectedCourtFilter === 'all' || court.id === selectedCourtFilter)
                      .map((court) => {
                        ...
                        return (
                          <button
                            key={court.id}
                            onClick={() => isAvailable ? openCreate(court.id, time) : openEdit(existingBooking)}
                            className={`p-2 rounded-lg text-sm font-medium transition-all hover:scale-105 ${
                              isAvailable
                                ? `${T.btnGhost} border-2 ${hasPromo ? 'border-yellow-400 bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20' : 'border-green-200 dark:border-green-700'}`
                          ...
                          >
                            ...
                            {!isAvailable && existingBooking && (
                              <div className="text-xs mt-1 truncate">
                                <div className="font-medium">{existingBooking.bookedByName || 'Occupato'}</div>
                                <div className="text-gray-400 dark:text-gray-500 text-[10px]">👆 Clicca per dettagli</div>
                              </div>
                            )}
                          </button>
                        );
