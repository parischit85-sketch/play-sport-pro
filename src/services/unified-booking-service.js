...
export async function createBooking(bookingData, user, options = {}) {
  ...
  console.log('🔍 Validating booking against', existingBookings.length, 'existing bookings');
...
async function loadCloudBookingsRaw() {
  ...
  const activeBookings = allBookings.filter(booking => {
    ...
    if (!isActive && Math.random() < 0.3) { // Log only ~30% of cancelled bookings to reduce noise
...
async function syncLocalWithCloud() {
  ...
  try {
    ...
    const activeCloudBookings = cloudBookings.filter(b => (b.status || BOOKING_STATUS.CONFIRMED) !== BOOKING_STATUS.CANCELLED);
...
export async function migrateOldData() {
  ...
  try {
    const oldLessons = localStorage.getItem('lessonBookings') || localStorage.getItem('lesson-bookings');
    if (oldLessons) {
      const parsed = JSON.parse(oldLessons);
      migrations.push(...parsed.map(l => ({
        ...l,
...
export function validateBooking(bookingData, existingBookings = []) {
  ...
  const conflicts = existingBookings.filter(booking => {
    ...
    const hasTimeOverlap = timeOverlaps(booking.time, booking.duration, bookingData.time, bookingData.duration);
...
export function wouldCreateHalfHourHole(courtId, date, startTime, duration, existingBookings) {
  console.log(`🔍 [HOLE DEBUG] Checking ${courtId} on ${date} at ${startTime} for ${duration}min`);
  console.log(`🔍 [HOLE DEBUG] Existing bookings count:`, existingBookings.length);
  ...
  console.log(`🔍 [HOLE DEBUG] Court ${courtId} bookings for ${date}:`, 
  ...
  if (nextBooking) {
    ...
    console.log(`🔍 [HOLE DEBUG] Gap AFTER: ${holeSize}min (${startTime} ends at ${Math.floor(bookingEnd/60)}:${String(bookingEnd%60).padStart(2,'0')}, next booking at ${nextBooking.time})`);
    
    if (holeSize > 0 && holeSize <= 30) {
      console.log(`⚠️ [HOLE DEBUG] Found problematic gap AFTER: ${holeSize}min`);
      ...
      if (prevBooking) {
        ...
        if (totalGap === 120) {
          console.log(`✅ [120MIN EXEMPTION] ${courtId} ${startTime}: 120min trapped slot allows ${holeSize}min hole`);
          return false;
        }
      }
      
      console.log(`🚫 [HOLE BLOCKED] ${courtId} ${startTime}: Would create ${holeSize}min hole AFTER booking`);
  ...
  if (prevBooking) {
    ...
    console.log(`🔍 [HOLE DEBUG] Gap BEFORE: ${holeSize}min (previous ends at ${Math.floor(prevEnd/60)}:${String(prevEnd%60).padStart(2,'0')}, our booking starts at ${startTime})`);
    
    if (holeSize > 0 && holeSize <= 30) {
      console.log(`⚠️ [HOLE DEBUG] Found problematic gap BEFORE: ${holeSize}min`);
      // Check EXEMPTION: is this part of a trapped slot of EXACTLY 120 minutes?
      const nextBooking = courtBookings.find(b => timeToMinutes(b.time) >= bookingEnd);
      ...
      console.log(`🚫 [HOLE BLOCKED] ${courtId} ${startTime}: Would create ${holeSize}min hole BEFORE booking`);
      return true; // Would create problematic hole without exemption
    }
  }
...
export function isSlotAvailable(courtId, date, startTime, duration, existingBookings) {
  console.log(`🔍 [SLOT AVAILABLE] Basic check for ${courtId} ${date} ${startTime} (${duration}min)`);
  
  const bookingStart = timeToMinutes(startTime);
  const bookingEnd = bookingStart + duration;
  
  console.log(`🕒 [SLOT AVAILABLE] Time range: ${bookingStart}min-${bookingEnd}min (${Math.floor(bookingStart/60)}:${String(bookingStart%60).padStart(2,'0')}-${Math.floor(bookingEnd/60)}:${String(bookingEnd%60).padStart(2,'0')})`);
  ...
  const hasConflict = existingBookings.some(booking => {
    ...
    if (overlap) {
      console.log(`⚠️ [SLOT AVAILABLE] Overlap found with ${booking.time} (${booking.duration}min): ${existingStart}min-${existingEnd}min`);
    }
    
    return overlap;
  });
  
  console.log(`📊 [SLOT AVAILABLE] Conflicts found: ${hasConflict}, Result: ${!hasConflict ? '✅ AVAILABLE' : '❌ BLOCKED'}`);
  return !hasConflict;
}
 ...
 */
export function isDurationBookable(courtId, date, startTime, duration, existingBookings, isUserBooking = false) {
  console.log(`🎮 [DURATION BOOKABLE] Checking ${courtId} ${date} ${startTime} (${duration}min) - isUserBooking: ${isUserBooking}`);
  ...
  if (isUserBooking) {
    const wouldCreateHole = wouldCreateHalfHourHole(courtId, date, startTime, duration, existingBookings);
    ...
    console.log(`🔓 [DURATION BOOKABLE] Admin booking - skipping hole prevention`);
  }
...
export async function searchBookingsForPlayer({ userId, email, name }) {
    ...
    return localBookings.filter(booking => {
      ...
      if (name) {
        ...
        if (Array.isArray(booking.players)) {
          return booking.players.some(player => 
  ...
  try {
    ...
    if (userId) {
      queries.push(getDocs(query(collection(db, COLLECTIONS.BOOKINGS), where("createdBy", "==", userId))));
    }
    
    if (email) {
      queries.push(getDocs(query(collection(db, COLLECTIONS.BOOKINGS), where("userEmail", "==", email))));
    }
    
    if (name) {
      // For name-based searches, we'll need to do client-side filtering since Firestore doesn't support complex queries
      queries.push(getDocs(query(collection(db, COLLECTIONS.BOOKINGS), where("bookedBy", "==", name))));
      queries.push(getDocs(query(collection(db, COLLECTIONS.BOOKINGS), where("players", "array-contains", name))));
    }
...
export async function getActiveUserBookings(userId) {
  ...
  if (!isCloudEnabled) {
    return (await getUserBookings({ uid: userId }, { includeHistory: true, includeCancelled: false }))
      .filter(booking => booking.date >= today && (booking.status || BOOKING_STATUS.CONFIRMED) === BOOKING_STATUS.CONFIRMED);
  }
...
export async function getUserBookingHistory(userId) {
  ...
  if (!isCloudEnabled) {
    return (await getUserBookings({ uid: userId }, { includeHistory: true, includeCancelled: true }))
