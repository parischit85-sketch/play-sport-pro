    ...
    () => ({
      ...
      championshipPoints: {
        ...
        knockoutProgressPoints: {
          round_of_16:
            tournament?.configuration?.championshipPoints?.knockoutProgressPoints?.round_of_16 ??
            10,
          quarter_finals:
            tournament?.configuration?.championshipPoints?.knockoutProgressPoints?.quarter_finals ??
            20,
          semi_finals:
            tournament?.configuration?.championshipPoints?.knockoutProgressPoints?.semi_finals ??
            40,
          finals:
            tournament?.configuration?.championshipPoints?.knockoutProgressPoints?.finals ?? 80,
          third_place:
            tournament?.configuration?.championshipPoints?.knockoutProgressPoints?.third_place ??
            15,
        },
      },
    }),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updates = {
        ...
        'configuration.defaultRankingForNonParticipants':
        ...
        'configuration.championshipPoints.groupPlacementPoints.1': Number(
          form.championshipPoints?.groupPlacementPoints?.[1] ?? 100
        ),
        'configuration.championshipPoints.groupPlacementPoints.2': Number(
          form.championshipPoints?.groupPlacementPoints?.[2] ?? 60
        ),
        'configuration.championshipPoints.groupPlacementPoints.3': Number(
          form.championshipPoints?.groupPlacementPoints?.[3] ?? 40
        ),
        'configuration.championshipPoints.groupPlacementPoints.4': Number(
          form.championshipPoints?.groupPlacementPoints?.[4] ?? 20
        ),
        'configuration.championshipPoints.knockoutProgressPoints.round_of_16': Number(
          form.championshipPoints?.knockoutProgressPoints?.round_of_16 ?? 10
        ),
        'configuration.championshipPoints.knockoutProgressPoints.quarter_finals': Number(
          form.championshipPoints?.knockoutProgressPoints?.quarter_finals ?? 20
        ),
        'configuration.championshipPoints.knockoutProgressPoints.semi_finals': Number(
          form.championshipPoints?.knockoutProgressPoints?.semi_finals ?? 40
        ),
        'configuration.championshipPoints.knockoutProgressPoints.finals': Number(
          form.championshipPoints?.knockoutProgressPoints?.finals ?? 80
        ),
        'configuration.championshipPoints.knockoutProgressPoints.third_place': Number(
          form.championshipPoints?.knockoutProgressPoints?.third_place ?? 15
        ),
      };
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Modifica Torneo</h3>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-white dark:bg-gray-800">
          ...
          <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
            ...
            {error && (
              <div className="p-3 rounded bg-red-50 border border-red-200 text-red-700 dark:bg-red-900/20 dark:border-red-800 dark:text-red-300 text-sm">
                {error}
              </div>
            )}
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setField('defaultRankingForNonParticipants', parseInt(e.target.value) || 1500)
                  }
                  className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                />
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Usato per i giocatori del circolo non iscritti al torneo quando selezionati nelle
                  squadre.
                </p>
              </div>
            </div>
          ...
          <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
            ...
            <div className="flex items-center gap-2 mt-3">
              ...
              />
              <label
                htmlFor="includeThirdPlaceMatch"
          ...
          <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
              ...
              <div className="mb-4 bg-red-50 dark:bg-red-900/20 border-2 border-red-300 dark:border-red-700 rounded-lg p-3 flex items-start gap-2">
                ...
                <div className="text-sm text-red-800 dark:text-red-200">
                  <div className="font-semibold mb-1">
                    🔒 Configurazione Bloccata - Punti Già Applicati
                  </div>
                  <p className="text-xs">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                />
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Somma dei delta RPA per match × moltiplicatore
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
        ...
        <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-lg bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600"
          >
            Annulla
          </button>
          <button
