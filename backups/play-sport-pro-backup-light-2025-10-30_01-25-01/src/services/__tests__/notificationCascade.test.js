...
describe('NotificationCascade', () => {
  ...
  describe('send()', () => {
    it('should send via push as first priority', async () => {
      ...
      // Mock push success
      vi.spyOn(notificationCascade, '_sendPush').mockResolvedValue({
        success: true,
        latency: 1500,
      });
    ...
    it('should fallback to email when push fails', async () => {
      ...
      vi.spyOn(notificationCascade, '_getUserPreferences').mockResolvedValue(null);
      vi.spyOn(notificationCascade, '_sendPush').mockResolvedValue({
        success: false,
        error: '410 Gone',
      });
      vi.spyOn(notificationCascade, '_sendEmail').mockResolvedValue({
        success: true,
        latency: 2300,
      });
    ...
    it('should try all channels in cascade until success', async () => {
      ...
      vi.spyOn(notificationCascade, '_sendSMS').mockResolvedValue({ success: true, latency: 3100 });
    ...
    it('should fail if all channels fail', async () => {
      ...
      vi.spyOn(notificationCascade, '_sendSMS').mockResolvedValue({
        success: false,
        error: 'SMS failed',
      });
      vi.spyOn(notificationCascade, '_sendInApp').mockResolvedValue({
        success: false,
        error: 'In-app failed',
      });
    ...
    it('should respect user preferences', async () => {
      ...
      vi.spyOn(notificationCascade, '_getUserPreferences').mockResolvedValue({
        pushEnabled: true,
        emailEnabled: false,
        smsEnabled: true,
      });
      ...
      vi.spyOn(notificationCascade, '_sendSMS').mockResolvedValue({ success: true, latency: 2800 });
  ...
  describe('Type-Specific Routing', () => {
    it('should include SMS for PAYMENT_DUE', async () => {
      ...
      vi.spyOn(notificationCascade, '_sendSMS').mockResolvedValue({ success: true, latency: 3200 });
  ...
  describe('Error Handling', () => {
    it('should handle channel timeout gracefully', async () => {
      ...
      vi.spyOn(notificationCascade, '_sendPush').mockRejectedValue(timeoutError);
      vi.spyOn(notificationCascade, '_sendEmail').mockResolvedValue({
        success: true,
        latency: 2200,
      });
