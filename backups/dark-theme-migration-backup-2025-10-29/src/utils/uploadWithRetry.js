 ...
 */
export async function uploadWithRetry(file, uploadPreset, maxRetries = 3, onProgress = null) {
...
export function validateFile(file, options = {}) {
  const { maxSizeMB = 5, allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'] } =
    options;
  ...
  if (sizeMB > maxSizeMB) {
    errors.push(`File too large (${sizeMB.toFixed(1)} MB). Maximum: ${maxSizeMB} MB`);
  }
