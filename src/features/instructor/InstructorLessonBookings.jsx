...
export default function InstructorLessonBookings({ compact = false }) {
    ...
    const loadInstructorBookings = async () => {
      ...
      try {
        ...
        const instructorBookings = allBookings.filter(booking =>
          ...
          (booking.isLessonBooking || booking.type === 'lesson' || booking.bookingType === 'lezione')
        );
  ...
  if (loading) {
    return (
      <div className={`bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl border border-white/20 dark:border-gray-700/20 p-6 shadow-2xl ${compact ? 'mb-4' : ''}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          {[
            ...
            { key: 'upcoming', label: 'Prossime', count: bookings.filter(b => new Date(b.date || new Date()) >= new Date()).length },
            { key: 'past', label: 'Passate', count: bookings.filter(b => new Date(b.date || new Date()) < new Date()).length }
          ].map(({ key, label, count }) => (
            <button
              key={key}
        ...
        {filteredBookings.length === 0 ? (
          <div className="text-center py-8 text-gray-500 dark:text-gray-400">
            ...
            <p className="text-lg font-medium mb-1">
              {filter === 'all' ? 'Nessuna lezione prenotata' :
               filter === 'upcoming' ? 'Nessuna lezione in programma' :
               'Nessuna lezione passata'}
            </p>
            <p className="text-sm">
              {filter === 'all' ? 'Le tue lezioni appariranno qui quando verranno prenotate' :
               filter === 'upcoming' ? 'Non hai lezioni programmate' :
          ...
          filteredBookings.map((booking) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(booking)} bg-current/10`}>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="font-medium text-gray-900 dark:text-white">
                    {booking.lessonType === 'individual' ? 'Lezione individuale' :
                     booking.lessonType === 'group' ? 'Lezione di gruppo' :
                     'Lezione'}
                  </span>
                </div>
                ...
                {booking.participants && booking.participants.length > 0 && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-medium text-gray-900 dark:text-white">
                      {booking.participants.length} {booking.participants.length === 1 ? 'persona' : 'persone'}
                    </span>
                  </div>
                )}
