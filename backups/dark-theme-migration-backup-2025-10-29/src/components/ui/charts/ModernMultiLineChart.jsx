...
export default function ModernMultiLineChart({
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div
          className={`bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl ${isMobile ? 'p-2 min-w-36' : 'p-4 min-w-48'}`}
        >
          <p
            className={`${isMobile ? 'text-xs' : 'text-sm'} font-semibold text-gray-900 dark:text-gray-100 ${isMobile ? 'mb-1' : 'mb-3'}`}
          >
            {label}
          </p>
              ...
              .map((entry, index) => (
                <div key={index} className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    />
                    <span
                      className={`${isMobile ? 'text-xs' : 'text-sm'} font-medium text-gray-700 dark:text-gray-300 ${isMobile ? 'truncate max-w-16' : ''}`}
                    >
