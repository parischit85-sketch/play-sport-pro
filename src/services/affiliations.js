...
import { collection, doc, query, where, getDocs, addDoc, updateDoc, deleteDoc, getDoc, orderBy, Timestamp } from 'firebase/firestore';
...
export async function isUserClubAdmin(userId, clubId) {
  try {
    ...
    const adminAffiliation = affiliations.find(a => 
      a.clubId === clubId && 
...
export async function userHasPermission(userId, clubId, permission) {
  try {
    const affiliations = await getUserAffiliations(userId);
    const affiliation = affiliations.find(a => 
      a.clubId === clubId && 
      a.status === AFFILIATION_STATUS.APPROVED
    );
