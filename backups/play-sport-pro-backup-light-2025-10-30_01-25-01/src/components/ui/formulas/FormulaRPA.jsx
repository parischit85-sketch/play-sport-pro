...
export function FormulaRPA({ sumA, sumB, gap, base, factor, gd, pts, winner, sets }) {
  ...
  return (
    <div className="space-y-3 text-sm">
      ...
      <div className="grid grid-cols-2 gap-4 p-3 bg-blue-50 bg-blue-900/20 rounded-lg">
        <div>
      ...
      {/* Step 2: Calcolo base */}
      <div className="p-3 bg-yellow-50 bg-yellow-900/20 rounded-lg">
        <div className="text-xs font-semibold text-yellow-700 text-yellow-300 mb-2">
          STEP 1: PUNTEGGIO BASE
        </div>
        <div className="text-sm font-mono">
          Base = (Team A + Team B) ÷ 100 = ({fmtInt(sumA)} + {fmtInt(sumB)}) ÷ 100 ={' '}
          <span className="font-bold text-yellow-700 text-yellow-300">{fmt2(base)}</span>
        </div>
        <div className="text-xs text-yellow-600 text-yellow-400 mt-1">
          Il punteggio base rappresenta il livello medio della partita
        </div>
      </div>

      {/* Step 3: Differenza games */}
      <div className="p-3 bg-purple-50 bg-purple-900/20 rounded-lg">
        <div className="text-xs font-semibold text-purple-700 text-purple-300 mb-2">
          STEP 2: BONUS/MALUS GAMES
        </div>
        <div className="text-sm">
          DG (Differenza Game) ={' '}
          <span className="font-bold text-purple-700 text-purple-300">{gd}</span>
        </div>
        <div className="text-xs text-purple-600 text-purple-400 mt-1">
          {gd > 0
      ...
      <div className="p-3 bg-green-50 bg-green-900/20 rounded-lg">
        <div className="text-xs font-semibold text-green-700 text-green-300 mb-2">
          STEP 3: MOLTIPLICATORE
        </div>
        <div className="text-sm font-mono">
          Punti = (Base + DG) × Factor = ({fmt2(base)} + {gd}) × {factor.toFixed(2)} ={' '}
          <span className="font-bold text-green-700 text-green-300">{fmt2(finalPoints)}</span>
        </div>
        <div className="text-xs text-green-600 text-green-400 mt-1">
      ...
      {/* Risultato finale */}
      <div className="p-3 bg-gray-100 bg-gray-800 rounded-lg border-2 border-gray-300 border-gray-600">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs font-semibold text-gray-600 text-gray-400">
              RISULTATO FINALE
            </div>
            <div className="text-lg font-bold text-gray-800 text-gray-200">
              Punti assegnati: {pts}
            </div>
          </div>
