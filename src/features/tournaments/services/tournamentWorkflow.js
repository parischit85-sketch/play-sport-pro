...
export class TournamentWorkflowManager {
  ...
  async startKnockoutStage() {
    try {
      ...
      if (qualifiedTeams.length === 0) {
        return {
          success: false,
          error: 'Nessuna squadra qualificata trovata. Assicurati che le classifiche siano state calcolate.',
        };
      }
      ...
      if (!isPowerOfTwo(qualifiedTeams.length)) {
        ...
        console.log('🧮 Non-power-of-two teams detected. Using BYE slots via manual bracket setup...');
        ...
        if (!manualResult.success) {
          return { success: false, error: manualResult.error || 'Errore nella creazione del tabellone con BYE' };
        }
  ...
  async assignThirdPlaceParticipants(matchId) {
    try {
      ...
      const thirdMatches = await getMatchesByRound(this.clubId, this.tournamentId, KNOCKOUT_ROUND.THIRD_PLACE);
      ...
      const loserName = match.winnerId === team1 ? (match.team2Name || null) : (match.team1Name || null);
      ...
      const thirdRef = doc(db, 'clubs', this.clubId, COLLECTIONS.TOURNAMENTS, this.tournamentId, COLLECTIONS.MATCHES, third.id);
  ...
  async advancePhase() {
    ...
    switch (checkResult.nextAction) {
      case 'openRegistration':
        return await updateTournamentStatus(this.clubId, this.tournamentId, TOURNAMENT_STATUS.REGISTRATION_OPEN);

      case 'closeRegistration':
        const closeResult = await updateTournamentStatus(this.clubId, this.tournamentId, TOURNAMENT_STATUS.REGISTRATION_CLOSED);
  ...
  try {
    ...
    if (result.success) {
      ...
      try {
        ...
        if (completedMatch?.type === 'knockout' && completedMatch?.round === KNOCKOUT_ROUND.SEMI_FINALS) {
