    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white bg-gray-800 border border-gray-200 border-gray-700 rounded-lg shadow-lg p-3">
          <p className="text-sm font-medium text-gray-900 text-gray-100 mb-2">{label}</p>
          {payload.map((entry, index) => (
            <p key={index} className="text-sm text-gray-600 text-gray-400">
  ...
  return (
    <div className="h-64 sm:h-80">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={last10Data} margin={{ left: 10, right: 10, top: 20, bottom: 20 }}>
          <CartesianGrid
            strokeDasharray="3 3"
            strokeOpacity={0.2}
            horizontal={true}
