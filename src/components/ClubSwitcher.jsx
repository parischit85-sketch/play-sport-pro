...
export default function ClubSwitcher({ className = '' }) {
  ...
  const approved = React.useMemo(() => {
    const approvedAffiliations = (userAffiliations || []).filter(a => a.status === AFFILIATION_STATUS.APPROVED);
  ...
  return (
    <div className={`relative ${className}`}>
      <select
        className="text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md px-2 py-1 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500"
        value={currentClubId || ''}
        onChange={(e) => selectClub(e.target.value)}
      >
        <option value="">Seleziona Club</option>
        {approved.map(a => (
          <option key={a.clubId} value={a.clubId}>{a.clubName || a.clubId}</option>
        ))}
      </select>
    </div>
  );
