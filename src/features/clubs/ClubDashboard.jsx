...
const QuickActionCard = ({ title, description, icon, onClick, gradient, iconBg }) => (
  <button
    ...
    className={
      ...
      gradient +
      ' border border-white/20 border-gray-700/30'
    }
  >
...
const ClubDashboard = () => {
  ...
  const { club, loading: clubLoading, isUserInstructor, playersLoaded } = useClub();
    ...
    if (user && clubId) {
      const loadActiveBookings = async () => {
        ...
        try {
          ...
          console.error('❌ [ClubDashboard] Error loading active bookings:', err);
        } finally {
  ...
  // Risoluzione robusta dei contatti con fallback per compatibilità dati legacy
  const phoneNumber = club.contact?.phone || club.phone || '';
  const whatsappNumber =
    club.contact?.whatsapp || club.whatsappNumber || club.whatsapp || phoneNumber;
  const googleMapsQuery =
    (club.location?.address || club.address?.street || '') +
    ', ' +
    (club.location?.city || club.city || '');
      ...
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className="flex items-center justify-between mb-4 md:mb-8">
          <div className="flex items-center gap-3 md:gap-4">
            {/* Logo */}
            <div className="w-12 h-12 md:w-16 md:h-16 rounded-full overflow-hidden border-2 border-white/20 bg-white shrink-0">
              {club.logoUrl ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-500 to-indigo-600 text-white">
                  <span className="text-xl md:text-2xl font-bold">{club.name.charAt(0)}</span>
                </div>
              )}
            </div>

            {/* Nome Club */}
            <h1 className="text-xl md:text-3xl font-bold text-white leading-tight">{club.name}</h1>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            {phoneNumber && (
              <a
                ...
                className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-green-500 hover:bg-green-600 text-white flex items-center justify-center shadow-lg transition-transform hover:scale-110"
                aria-label="Chiama"
              >
                <svg
            ...
            {whatsappNumber && (
              <a
                ...
                className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-[#25D366] hover:bg-[#128C7E] text-white flex items-center justify-center shadow-lg transition-transform hover:scale-110"
                aria-label="WhatsApp"
              >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-blue-500 hover:bg-blue-600 text-white flex items-center justify-center shadow-lg transition-transform hover:scale-110"
