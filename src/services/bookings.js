...
export const BOOKING_CONFIG = {
  ...
  // Configurazione lezioni con maestro
  // NOTA (2025-09): i prezzi qui sono già il TOTALE della lezione (non per partecipante)
  // Quindi il costo per partecipante è: total / participants (solo a scopo display)
  // La vecchia logica con participantMultipliers è deprecata.
  lesson: {
    basePrice60: 30, // Prezzo totale lezione 60'
    basePrice90: 45, // Prezzo totale lezione 90'
    // participantMultipliers DEPRECATO: lasciato solo per backward compatibility se presente.
    // Se true il prezzo lezione sostituisce quello del campo; se false si somma al prezzo campo (scenario misto)
    replaceCourtPrice: true,
  },
...
// Calcola prezzo lezione utilizzando le configurazioni esistenti degli istruttori
export function calculateLessonPrice({
  ...
  const lessonCfg = BOOKING_CONFIG.lesson;
  if (!lessonCfg) return 0;

  // 1. Utilizza i prezzi specifici dell'istruttore configurati nel sistema
  let baseTotal;

  if (instructor?.instructorData) {
    // Mappa il numero di partecipanti ai prezzi configurati per l'istruttore
    switch (participants) {
      case 1:
        if (instructor.instructorData.priceSingle > 0) {
          baseTotal = Number(instructor.instructorData.priceSingle);
        }
        break;
      case 2:
        if (instructor.instructorData.priceCouple > 0) {
          baseTotal = Number(instructor.instructorData.priceCouple);
        }
        break;
      case 3:
        if (instructor.instructorData.priceThree > 0) {
          baseTotal = Number(instructor.instructorData.priceThree);
        }
        break;
      case 4:
        // Per 4 partecipanti, usiamo priceMatchLesson se disponibile, altrimenti hourlyRate
        if (instructor.instructorData.priceMatchLesson > 0) {
          baseTotal = Number(instructor.instructorData.priceMatchLesson);
        }
        break;
    }
  ...
  if (!lessonCfg.replaceCourtPrice && court) {
    // Somma il prezzo del campo (senza extra) se configurato così
    total += calculatePrice(court, duration, false, false);
  }

  // Extra luce / riscaldamento
  if (lighting && court?.hasLighting) total += BOOKING_CONFIG.pricing.lighting;
  if (heating && court?.hasHeating) total += BOOKING_CONFIG.pricing.heating;
