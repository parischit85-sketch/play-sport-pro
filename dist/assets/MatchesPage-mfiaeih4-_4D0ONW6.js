...
import { r as k, b as D, c as oe } from './router-mfiaeih4-DNPpahz0.js';
import { S as Z } from './Section-mfiaeih4-DiH4h-T9.js';
import { s as H, b as xe } from './names-mfiaeih4-BW9lV2zG.js';
import './vendor-mfiaeih4-D3F3s8fL.js';
import './firebase-mfiaeih4-X_I_guKF.js';
function me({ m: a, playersById: t, onShowFormula: r, onDelete: d, T: i }) {
  const [s, x] = k.useState(!1),
    n = (j) => t?.[j]?.name ?? j,
    l = `${H(n(a.teamA[0]))} & ${H(n(a.teamA[1]))}`,
    G = `${H(n(a.teamB[0]))} & ${H(n(a.teamB[1]))}`,
    m = `${n(a.teamA[0])} & ${n(a.teamA[1])}`,
    $ = `${n(a.teamB[0])} & ${n(a.teamB[1])}`,
    c = a.winner === 'A',
    E = c
...
function he({ sumA: a, sumB: t, teamALabel: r, teamBLabel: d }) {
...
function ge({
  ...
  return e.jsxs('div', {
    ...
    children: [
      ...
      e.jsxs('div', {
        className: 'text-neutral-500',
        children: ['Risultato set: ', m.setsA, '-', m.setsB, ' (', ue(n), ')'],
      }),
    ],
  });
}
...
function je({ state: a, setState: t, playersById: r, onShowFormula: d, derivedMatches: i, T: s }) {
  const x = a.players,
    n = k.useMemo(() => [...x].sort(xe), [x]),
    [l, G] = k.useState(''),
    [m, $] = k.useState(''),
    [c, E] = k.useState(''),
    [h, S] = k.useState(''),
    ...
    q = () => {
      ...
      if (!B.winner || L == null || z == null) {
        d(e.jsx(he, { sumA: L, sumB: z, teamALabel: `${o} + ${p}`, teamBLabel: `${u} + ${w}` }));
        return;
      }
        ...
        children: [
          ...
          x &&
            e.jsx('div', {
              ...
              children: e.jsx('div', {
                className:
                  'bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full max-h-[80vh] overflow-y-auto border dark:border-gray-600',
                children: e.jsxs('div', {
                  className: 'p-6',
                  children: [
                    e.jsxs('div', {
