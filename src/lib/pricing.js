...
/**
 * Ritorna true se, per il campo specificato, esiste una fascia (timeSlot)
 * attiva che include la data/ora passata. Se il campo non ha timeSlots,
 * consideriamo il campo NON prenotabile (richiede configurazione esplicita).
 */
export function isCourtBookableAt(date, courtId, courts = null) {
  const courtsData = courts || [];
  const court = courtsData.find((c) => c.id === courtId);
  if (!court?.timeSlots || court.timeSlots.length === 0) return false;
  return Boolean(findActiveTimeSlot(date, courtId, courtsData));
}
...
export function getRateInfo(date, cfg, courtId, courts = null) {
  ...
  if (disc)
    return { rate: Number(disc.eurPerHour) || 0, source: 'legacy', rule: disc, isPromo: false };
  const full = (pricing.full || []).find((r) => ruleMatches(date, r, courtId));
  if (full)
    return { rate: Number(full.eurPerHour) || 0, source: 'legacy', rule: full, isPromo: false };
