        ...
        <div className="grid grid-cols-2 gap-4 mb-4 text-center">
          ...
          {champPoints !== null && (
            <div>
              ...
              <div className={`text-xs ${T.subtext}`}>Punti Campionato{champEntries ? ` · ${champEntries}` : ''}</div>
            </div>
          )}
          <div>
    ...
    <>
      ...
      {balance > 0 && (
        <div className="text-center w-[90px] shrink-0">
          <div className="font-semibold text-green-600 dark:text-green-400">€{balance.toFixed(2)}</div>
          <div className={`text-xs ${T.subtext}`}>Credito</div>
        </div>
      )}
      ...
      {champPoints !== null && (
        <div className="text-center w-[110px] shrink-0">
          <div className="font-semibold text-amber-600 dark:text-amber-400">{champPoints.toFixed(1)}</div>
          <div className={`text-xs ${T.subtext}`}>Punti Campionato{champEntries ? ` · ${champEntries}` : ''}</div>
        </div>
      )}
