  ...
  calculateStats(timeRange = '24h') {
    ...
    return {
      pageLoads: {
        ...
        p95LoadTime: this.percentile(
          recentPageLoads.map((m) => m.loadTime),
          95
        ),
        budgetViolations: recentPageLoads.filter((m) => m.loadTime > this.budgets.pageLoadTime)
          .length,
      },
      apiCalls: {
      ...
      errors: {
        ...
        errorRate:
          recentPageLoads.length > 0 ? (recentErrors.length / recentPageLoads.length) * 100 : 0,
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white bg-gray-800 rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="p-6 border-b border-gray-200 border-gray-700 bg-gradient-to-r from-blue-50 to-cyan-50 from-blue-900/20 to-cyan-900/20">
          ...
          <div className="flex gap-3 mt-4 flex-wrap">
            <select
              ...
              className="px-3 py-2 bg-white bg-gray-700 border border-gray-300 border-gray-600 rounded-lg text-sm"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 from-blue-900/20 to-blue-800/20 rounded-lg p-4 border border-blue-200 border-blue-700">
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-semibold text-blue-900 text-blue-300">Page Loads</h3>
              ...
              <div className="text-xs text-blue-600 text-blue-500 mt-1">
                P95: {formatMs(stats.pageLoads.p95LoadTime)}
              </div>
              {stats.pageLoads.budgetViolations > 0 && (
                <div className="text-xs text-red-600 text-red-400 mt-2 font-semibold">
            ...
            <div className="bg-gradient-to-br from-green-50 to-green-100 from-green-900/20 to-green-800/20 rounded-lg p-4 border border-green-200 border-green-700">
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-semibold text-green-900 text-green-300">API Calls</h3>
                <span className="text-2xl">🌐</span>
              </div>
              ...
              <div className="text-xs text-green-600 text-green-500 mt-1">
            ...
            <div
              className={`bg-gradient-to-br rounded-lg p-4 border ${
                stats.errors.count > 0
                  ? 'from-red-50 to-red-100 from-red-900/20 to-red-800/20 border-red-200 border-red-700'
                  : 'from-gray-50 to-gray-100 from-gray-900/20 to-gray-800/20 border-gray-200 border-gray-700'
              }`}
            >
              <div className="flex items-center justify-between mb-2">
                <h3
                  className={`font-semibold ${stats.errors.count > 0 ? 'text-red-900 text-red-300' : 'text-gray-900 text-gray-300'}`}
                >
                  Errors
                </h3>
              ...
              <div
                className={`text-3xl font-bold ${stats.errors.count > 0 ? 'text-red-900 text-red-100' : 'text-gray-900 text-gray-100'}`}
              >
                {stats.errors.count}
              </div>
              <div
                className={`text-sm mt-1 ${stats.errors.count > 0 ? 'text-red-700 text-red-400' : 'text-gray-700 text-gray-400'}`}
              >
                Error Rate: {stats.errors.errorRate.toFixed(2)}%
              </div>
              <div
                className={`text-xs mt-1 ${stats.errors.count > 0 ? 'text-red-600 text-red-500' : 'text-gray-600 text-gray-500'}`}
              >
            ...
            <div
              className={`bg-gradient-to-br rounded-lg p-4 border ${
                stats.firestore.estimatedCost.withinFreeTier
                  ? 'from-purple-50 to-purple-100 from-purple-900/20 to-purple-800/20 border-purple-200 border-purple-700'
                  : 'from-orange-50 to-orange-100 from-orange-900/20 to-orange-800/20 border-orange-200 border-orange-700'
              }`}
            >
              <div className="flex items-center justify-between mb-2">
                <h3
                  className={`font-semibold ${stats.firestore.estimatedCost.withinFreeTier ? 'text-purple-900 text-purple-300' : 'text-orange-900 text-orange-300'}`}
                >
                  Firestore
                </h3>
                <span className="text-2xl">🔥</span>
              ...
              <div
                className={`text-3xl font-bold ${stats.firestore.estimatedCost.withinFreeTier ? 'text-purple-900 text-purple-100' : 'text-orange-900 text-orange-100'}`}
              >
                ${stats.firestore.estimatedCost.total.toFixed(4)}
              </div>
              <div
                className={`text-sm mt-1 ${stats.firestore.estimatedCost.withinFreeTier ? 'text-purple-700 text-purple-400' : 'text-orange-700 text-orange-400'}`}
              >
                Reads: {stats.firestore.reads.toLocaleString()}
              </div>
              <div
                className={`text-xs mt-1 ${stats.firestore.estimatedCost.withinFreeTier ? 'text-purple-600 text-purple-500' : 'text-orange-600 text-orange-500'}`}
              >
                Writes: {stats.firestore.writes.toLocaleString()}
              </div>
              {!stats.firestore.estimatedCost.withinFreeTier && (
                <div className="text-xs text-orange-600 text-orange-400 mt-2 font-semibold">
                  ⚠️ Over free tier!
                </div>
              )}
            </div>
          ...
          <div className="bg-gray-50 bg-gray-900 rounded-lg p-4 mb-6">
            <h3 className="font-bold text-gray-900 text-white mb-3">📏 Performance Budgets</h3>
            <div className="space-y-3">
              <div>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span
                    className={
                      stats.pageLoads.avgLoadTime > budgets.pageLoadTime
                ...
                <div className="h-2 bg-gray-200 bg-gray-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (stats.pageLoads.avgLoadTime / budgets.pageLoadTime) * 100)}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span
                    className={
                      stats.firestore.reads > budgets.firestoreReadsPerDay
                        ? 'text-red-600 font-semibold'
                        : 'text-green-600'
                    }
                  >
                    {stats.firestore.reads.toLocaleString()} /{' '}
                    {budgets.firestoreReadsPerDay.toLocaleString()}
                  </span>
                </div>
                <div className="h-2 bg-gray-200 bg-gray-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (stats.firestore.reads / budgets.firestoreReadsPerDay) * 100)}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span
                    className={
                      stats.firestore.writes > budgets.firestoreWritesPerDay
                        ? 'text-red-600 font-semibold'
                        : 'text-green-600'
                    }
                  >
                    {stats.firestore.writes.toLocaleString()} /{' '}
                    {budgets.firestoreWritesPerDay.toLocaleString()}
                  </span>
                </div>
                <div className="h-2 bg-gray-200 bg-gray-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (stats.firestore.writes / budgets.firestoreWritesPerDay) * 100)}%`,
                    }}
                  />
                </div>
              </div>
