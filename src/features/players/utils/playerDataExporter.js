...
export function preparePlayerDataForExport(player, additionalData = {}) {
  ...
  const hasAddress = Object.values(address).some(
    (v) => v && String(v).length > 0 && v !== 'Italia'
  );
  ...
  return {
    ...
    medicalCertificate: player.medicalCertificate
      ? {
          hasValidCertificate:
            player.medicalCertificate.isValid ||
            player.medicalCertificate.status === 'valid' ||
            false,
          // Support both expirationDate and expiryDate keys
          expirationDate:
            player.medicalCertificate.expirationDate ||
            player.medicalCertificate.expiryDate ||
            null,
    ...
    // ===== WALLET (se incluso) =====
    wallet:
      additionalData.wallet || player.wallet
        ? {
    ...
    bookings:
      additionalData.bookings || player.bookings
        ? {
...
export function downloadFile(content, filename, mimeType) {
  const isTest =
    ...
    (typeof globalThis !== 'undefined' &&
      ...
      (globalThis.process.env.NODE_ENV === 'test' || globalThis.process.env.VITEST_WORKER_ID)) ||
  ...
  const parent =
    typeof document !== 'undefined' ? document.body || document.documentElement || document : null;
  let appended = false;
  try {
  ...
  try {
    ...
    setTimeout(() => {
      try {
        if (appended && parent && typeof parent.removeChild === 'function')
...
export function downloadPlayerJSON(player, additionalData = {}) {
  ...
  const base =
    first || last
      ? `giocatore_${first}_${last}`
      : `giocatore_${sanitizeFilenamePart(safePlayer.id)}`;
...
export function generatePlayerReport(player, additionalData = {}) {
  ...
  if (data.personalInfo.birthDate)
    report += `Data di nascita: ${String(data.personalInfo.birthDate)}\n`;
  if (data.personalInfo.placeOfBirth)
    report += `Luogo di nascita: ${data.personalInfo.placeOfBirth}\n`;
  ...
  if (data.metadata && (data.metadata.createdAt || data.metadata.updatedAt)) {
    ...
    if (data.metadata.updatedAt)
      report += `Ultimo aggiornamento: ${fmt(data.metadata.updatedAt)}\n`;
  }
  ...
  if (data.medicalCertificate) {
    ...
    if (data.medicalCertificate.expirationDate)
      report += `Scadenza: ${data.medicalCertificate.expirationDate}\n`;
    if (data.medicalCertificate.issueDate)
      report += `Data rilascio: ${data.medicalCertificate.issueDate}\n`;
    if (data.medicalCertificate.certificateType)
      report += `Tipo: ${data.medicalCertificate.certificateType}\n`;
    if (data.medicalCertificate.doctorName)
      report += `Medico: ${data.medicalCertificate.doctorName}\n`;
  ...
  if (data.wallet) {
    ...
    const sumCredits = tx.reduce(
      (acc, t) => acc + (t && t.type === 'credit' && typeof t.amount === 'number' ? t.amount : 0),
      0
    );
    const sumPositive = tx.reduce(
      (acc, t) => acc + (t && typeof t.amount === 'number' && t.amount > 0 ? t.amount : 0),
      0
    );
    const creditsValue =
      (typeof data.wallet.credits === 'number' && data.wallet.credits) ||
