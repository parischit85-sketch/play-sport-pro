    ...
    <div className="relative bg-gradient-to-br from-white via-gray-50 to-blue-50 dark:from-gray-900 dark:via-gray-800 dark:to-blue-900/20 border-b-2 border-gray-200 dark:border-gray-700">
      ...
      <div className="relative px-6 py-6">
        ...
        <div className="mb-6 relative z-20">
          <h2 id="player-details-title" className="text-3xl font-bold text-gray-900 dark:text-white mb-1">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {player.category && (
              <span className={`inline-flex items-center gap-1 px-2.5 py-1 ${getCategoryColor(player.category)} text-white text-xs font-semibold rounded-md shadow-sm`}>
            ...
            {player.email && (
              <span className={`text-sm ${T?.subtext || 'text-gray-500'}`}>
                📧 {player.email}
              </span>
            )}
          </div>
                ...
                <div className="relative">
                  ...
                  {player.tournamentData?.isParticipant && player.tournamentData?.isActive ? (
                    <>
                      ...
                      {rankingDelta !== 0 && (
                        <div className={`text-xs font-semibold flex items-center gap-1 ${rankingDelta > 0 ? 'text-green-300' : 'text-red-300'}`}>
                          <span>{rankingDelta > 0 ? '↗' : '↘'}</span>
                          <span>{rankingDelta > 0 ? '+' : ''}{rankingDelta}</span>
                        </div>
                      )}
                    </>
                  ) : (
