                      ...
                      <span
                        className={`px-1.5 sm:px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-bold ${
                          activeTab === key
                            ...
                            : 'bg-indigo-100 bg-indigo-900/30 text-indigo-700 text-indigo-300'
                        }`}
                      >
                        {count}
                      </span>
  ...
  if (bookings.length === 0) {
    return (
      <div className="text-center py-12">
        <svg
          className="w-16 h-16 mx-auto mb-4 text-gray-400 text-gray-600"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex-1">
              <div className="text-sm font-semibold text-gray-900 text-white mb-1 flex items-center gap-2">
                <svg
          ...
          <div className="space-y-2">
            ...
            {booking.price && (
              <div className="text-sm text-green-600 text-green-400 font-medium">
                €{booking.price}
              </div>
            )}
          </div>
  ...
  if (timeSlots.length === 0) {
    return (
      <div className="text-center py-16">
        <div className="w-20 h-20 mx-auto mb-6 bg-gradient-to-br from-purple-100 to-blue-100 from-purple-900/30 to-blue-900/30 rounded-full flex items-center justify-center">
          <svg
            className="w-10 h-10 text-purple-500 text-purple-400"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      >
          ...
          <div className="flex items-start justify-between mb-4 pb-4 border-b border-gray-700 gap-3">
            ...
            <div className="flex flex-col gap-1.5 flex-shrink-0">
              ...
              {!isExpired && slot.isActive !== false && (
                <span className="px-2 sm:px-3 py-1 sm:py-1.5 bg-emerald-100 bg-emerald-900/30 text-emerald-700 text-emerald-400 text-xs sm:text-sm font-bold rounded-full flex items-center gap-1 sm:gap-1.5">
              ...
              {isExpired && (
                <span className="px-2 sm:px-3 py-1 sm:py-1.5 bg-red-100 bg-red-900/30 text-red-700 text-red-400 text-xs sm:text-sm font-bold rounded-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 md:gap-6 mb-4 sm:mb-6">
          ...
          <div className="bg-gradient-to-br from-orange-900/20 to-red-900/20 rounded-lg p-3 sm:p-4 border border-orange-700/30">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs sm:text-sm font-semibold text-orange-900 text-orange-200">
                Orario
              </span>
            </div>
            <p className="text-lg sm:text-xl font-bold text-orange-900 text-orange-100">
          ...
          <div className="bg-gradient-to-br from-green-900/20 to-emerald-900/20 rounded-lg p-3 sm:p-4 border border-green-700/30">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs sm:text-sm font-semibold text-green-900 text-green-200">
                Maestro
              </span>
            </div>
            <p className="text-sm sm:text-base font-bold text-green-900 text-green-100 truncate">
              {user?.displayName || 'Tu'}
            </p>
          </div>
          ...
          <div className="bg-gradient-to-br from-purple-900/20 to-indigo-900/20 rounded-lg p-3 sm:p-4 border border-purple-700/30 sm:col-span-2 md:col-span-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs sm:text-sm font-semibold text-purple-900 text-purple-200">
                Campi Selezionati
              </span>
            </div>
                ...
                Array.isArray(club?.courts) ? (
                  club.courts
                    ...
                    .map((court) => (
                      <span
                        key={court.id}
                        className="px-2 py-0.5 sm:py-1 bg-purple-200 bg-purple-800/50 text-purple-900 text-purple-100 text-xs font-semibold rounded"
                      >
                        {court.name}
                      </span>
                    ))
                ) : (
                  <span className="text-xs sm:text-sm font-medium text-purple-900 text-purple-100">
                    {slot.courtIds.length} {slot.courtIds.length === 1 ? 'campo' : 'campi'}
                  </span>
                )
              ) : (
                <span className="text-xs sm:text-sm text-purple-700 text-purple-300">
        ...
        {(slot.maxBookings || slot.maxParticipants || slot.price > 0) && (
          <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-3 sm:mb-4 text-xs sm:text-sm text-gray-400">
            ...
            {slot.price > 0 && (
              <div className="flex items-center gap-1.5 font-semibold text-green-600 text-green-400">
                <svg
                  className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0"
                  fill="none"
                  stroke="currentColor"
      ...
      {activeTimeSlots.length > 0 ? (
        <div className="space-y-3 sm:space-y-4">
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            <h3 className="text-lg sm:text-xl font-bold text-gray-900 text-gray-100">
              <span className="hidden sm:inline">Fasce Orarie</span>
              <span className="sm:hidden">Fasce</span>
            </h3>
            <span className="px-2 sm:px-3 py-1 bg-green-100 bg-green-900/30 text-green-700 text-green-400 text-xs sm:text-sm font-bold rounded-full">
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        {/* Date Selector */}
        <div>
          <label className="block text-sm font-semibold text-gray-900 text-gray-100 mb-2">
            Data
          </label>
          <input
            type="date"
        ...
        <div className={`p-3 rounded-lg ${T.cardBg} border-l-4 border-green-500`}>
          <div className="flex items-center gap-2">
            <svg
              className="w-5 h-5 text-green-600 text-green-400"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        <div className="flex gap-3 pt-4">
          <button
            type="button"
            onClick={onClose}
            className={`flex-1 py-3 px-4 ${T.cardBg} ${T.border} rounded-lg hover:bg-gray-50 hover:bg-gray-700 ${T.text} font-semibold transition-colors text-base`}
          >
            Annulla
          </button>
          <button
