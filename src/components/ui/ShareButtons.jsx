...
export default function ShareButtons({ title, url, captureRef, captionBuilder, size = 'sm', T }) {
  ...
  const caption =
    (typeof captionBuilder === 'function' ? captionBuilder() : '') || title || 'Sporting Cat';
  ...
  return (
    <div className="flex items-center gap-1 sm:gap-2 flex-wrap">
      <button type="button" onClick={shareNative} className={Btn} title="Condividi">
        <span className="sm:hidden">📤</span>
        <span className="hidden sm:inline">Condividi</span>
      </button>
      <button type="button" onClick={shareFacebook} className={Btn} title="Condividi su Facebook">
        <span className="sm:hidden">f</span>
        <span className="hidden sm:inline">Facebook</span>
      </button>
      <button type="button" onClick={shareInstagram} className={Btn} title="Condividi su Instagram">
        <span className="sm:hidden">📸</span>
