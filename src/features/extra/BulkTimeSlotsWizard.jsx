...
// ============================================
export function BulkTimeSlotsWizard({ isOpen, onClose, onApply, courts = [], selectedCourtIndices = [], T }) {
  ...
  const targetCourts = selectedCourtIndices.length > 0
  ...
  const generatePreviewSlots = () => {
    const slots = [];
    const { startTime, endTime, pattern, customInterval, selectedDays, basePrice, pricingStrategy, peakPrice, offPeakPrice, peakHours, slotLabel, autoNumber, isPromo } = wizardData;
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                Step {currentStep} di 5 - {targetCourts.length} {targetCourts.length === 1 ? 'campo' : 'campi'} selezionati
              </p>
            </div>
            <button
              onClick={onClose}
              ...
              {wizardData.pattern === 'custom' && (
                <div className="mt-4">
                  ...
                  <input
                    ...
                    onChange={(e) => setWizardData({ ...wizardData, customInterval: parseInt(e.target.value) || 60 })}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mt-4">
                ...
                <div className="text-sm text-blue-800 dark:text-blue-300">
                  <p>• Range: {wizardData.startTime} - {wizardData.endTime}</p>
                  <p>• Durata: {Math.floor((parseInt(wizardData.endTime.split(':')[0]) * 60 + parseInt(wizardData.endTime.split(':')[1]) - (parseInt(wizardData.startTime.split(':')[0]) * 60 + parseInt(wizardData.startTime.split(':')[1]))) / 60)} ore e {(parseInt(wizardData.endTime.split(':')[0]) * 60 + parseInt(wizardData.endTime.split(':')[1]) - (parseInt(wizardData.startTime.split(':')[0]) * 60 + parseInt(wizardData.startTime.split(':')[1]))) % 60} minuti</p>
                  <p>• Fasce previste: ~{Math.floor((parseInt(wizardData.endTime.split(':')[0]) * 60 + parseInt(wizardData.endTime.split(':')[1]) - (parseInt(wizardData.startTime.split(':')[0]) * 60 + parseInt(wizardData.startTime.split(':')[1]))) / (wizardData.pattern === 'hourly' ? 60 : wizardData.pattern === 'halfHourly' ? 30 : wizardData.customInterval))}</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2 mt-4">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-300 rounded-lg text-sm font-medium hover:bg-purple-200 dark:hover:bg-purple-800"
                >
                  Weekend
                </button>
                <button
                  onClick={() => setWizardData({ ...wizardData, selectedDays: [0, 1, 2, 3, 4, 5, 6] })}
                  className="px-4 py-2 bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 rounded-lg text-sm font-medium hover:bg-green-200 dark:hover:bg-green-800"
                >
                  Tutti
                </button>
              </div>
              ...
              {wizardData.pricingStrategy === 'uniform' && (
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setWizardData({ ...wizardData, basePrice: parseFloat(e.target.value) || 0 })}
                    className={`${T.input} w-full`}
                  />
                </div>
              )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setWizardData({ ...wizardData, peakPrice: parseFloat(e.target.value) || 0 })}
                        className={`${T.input} w-full`}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setWizardData({ ...wizardData, offPeakPrice: parseFloat(e.target.value) || 0 })}
                        className={`${T.input} w-full`}
                      />
                    </div>
                  </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <label htmlFor="autoNumber" className="text-sm text-gray-700 dark:text-gray-300">
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="w-4 h-4 text-blue-600 rounded"
                  />
                  <label htmlFor="isPromo" className="text-sm text-gray-700 dark:text-gray-300">
        ...
        <div className="p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
          <div className="flex gap-3 justify-between">
            ...
            <div className="flex gap-3">
              <button
                onClick={onClose}
                className="px-6 py-3 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 font-medium transition-colors"
              >
                Annulla
              </button>
