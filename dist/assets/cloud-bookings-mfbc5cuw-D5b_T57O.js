...
import { p as n } from './index-mfbc5cuw-B5KdbvAg.js';
import './vendor-mfbc5cuw-D3F3s8fL.js';
import './router-mfbc5cuw-tw9hraEf.js';
const c = 'bookings';
async function w() {
  try {
    const r = p(l(n, c), a('status', '==', 'confirmed'), i('date', 'asc'), i('time', 'asc'));
    return (await u(r)).docs.map((t) => ({ id: t.id, ...t.data() }));
  } catch (r) {
    throw (
...
async function I(r) {
  try {
    ...
    return (await u(t)).docs.map((s) => ({ id: s.id, ...s.data() }));
  } catch (e) {
    return (
...
async function E(r, e, t) {
  try {
    const o = { ...e, updatedAt: d(), updatedBy: t?.uid || null };
    return (await m(f(n, c, r), o), { id: r, ...e, updatedAt: new Date().toISOString() });
  } catch (o) {
    throw (
...
export {
  v as cancelCloudBooking,
  A as createCloudBooking,
  O as getPublicBookings,
  I as loadActiveUserBookings,
  S as loadBookingHistory,
  w as loadPublicBookings,
  E as updateCloudBooking,
};
