...
function calculateInstructorsAvailableToday(instructors, lessonConfig, bookings, lessons, today) {
  ...
  const lessonsWithoutInstructor = allTodayBookings.filter(b => 
    b.isLessonBooking === true && !b.instructorId
  );
  if (lessonsWithoutInstructor.length > 0) {
    console.warn('⚠️ Found lessons without instructorId (manual bookings):', 
  ...
  lessonConfig.timeSlots.forEach((configSlot) => {
    ...
    if (configSlot.selectedDates && Array.isArray(configSlot.selectedDates) && configSlot.selectedDates.length > 0) {
    ...
    for (let minutes = startTotalMinutes; minutes < endTotalMinutes; minutes += 60) {
      ...
      instructorIds.forEach(instructorId => {
        ...
        console.log(`🔍 [calculateInstructorsAvailableToday] Checking instructor ${instructor.name} (${instructorId}) for slot ${timeString}`);
        ...
        const conflictingBookings = allTodayBookings.filter(b => {
          const bookingStartMinutes = parseInt(b.time.split(':')[0]) * 60 + parseInt(b.time.split(':')[1]);
        ...
        console.log(`  📅 Found ${conflictingBookings.length} bookings at ${timeString}:`, 
        ...
        console.log(`  ✅ Instructor ${instructor.name} available at ${timeString}: ${!hasConflict}`);
