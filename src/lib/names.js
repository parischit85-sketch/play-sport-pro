...
export const IT_COLLATOR = new Intl.Collator("it", {
  sensitivity: "base",
  usage: "sort",
  numeric: true,
});
const firstKey = (fullName = "") => {
  const parts = String(fullName).trim().split(/\s+/);
  const first = parts[0] || "";
  const rest = parts.slice(1).join(" ");
  return `${first} ${rest}`.trim();
};
export const byPlayerFirstAlpha = (a, b) =>
  IT_COLLATOR.compare(firstKey(a.name), firstKey(b.name));
