...
export class NotificationCascade {
  ...
  async send(userId, notification, options = {}) {
    ...
    try {
      ...
      console.log(
        `📤 [NotificationCascade] Attempting delivery for user ${userId} via channels: ${channels.join(' → ')}`
      );
      ...
      for (const channel of channels) {
        ...
        try {
          ...
          if (result.success) {
            ...
            console.log(
              `✅ [NotificationCascade] Delivered via ${channel} in ${latency}ms (cost: €${cost.toFixed(4)})`
            );
  ...
  async sendViaEmail(userId, notification, userPreferences) {
    try {
      ...
      if (
        ...
        (Array.isArray(userPreferences?.disabledChannels) &&
          userPreferences.disabledChannels.includes('email')) ||
        (np.channels && typeof np.channels === 'object' && np.channels.email === false) ||
        (userPreferences?.channels &&
          typeof userPreferences.channels === 'object' &&
  ...
  determineChannels(notification, userPreferences, options) {
    ...
    const isChannelEnabled = (ch) => {
      ...
      if (Array.isArray(prefs.disabledChannels) && prefs.disabledChannels.includes(ch))
        return false;
  ...
  getStats() {
    return {
      ...
      averageCost: this.stats.succeeded > 0 ? this.stats.totalCost / this.stats.succeeded : 0,
      channelEfficiency: Object.keys(this.stats.byChannel).map((channel) => ({
        ...
        successRate:
          this.stats.byChannel[channel].attempts > 0
            ? (
                (this.stats.byChannel[channel].successes / this.stats.byChannel[channel].attempts) *
                100
              ).toFixed(2) + '%'
            : 'N/A',
      })),
