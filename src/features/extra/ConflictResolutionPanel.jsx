  ...
  switch (action.type) {
    case 'merge': {
      ...
      updatedSlots = updatedSlots.filter(
        (slot) => !action.deleteSlots.includes(slot.id)
      );
...
export function ConflictResolutionPanel({
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                {conflicts.length} {conflicts.length === 1 ? 'conflitto rilevato' : 'conflitti rilevati'} in "{court?.name}"
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          {conflicts.length === 0 ? (
            ...
            <div className="space-y-6">
              {conflictSuggestions.map(({ conflict, conflictIndex, suggestions }) => (
                <div
                  key={conflictIndex}
                  className="border-2 border-red-200 dark:border-red-800 rounded-xl p-4 bg-red-50/50 dark:bg-red-900/10"
                >
                  {/* Conflict Info */}
                  <div className="mb-4 pb-4 border-b border-red-200 dark:border-red-800">
                    <h3 className="font-bold text-lg text-red-900 dark:text-red-300 mb-2">
