...
export const usePlayerPermissions = (player = null) => {
  ...
  const permissions = useMemo(() => {
    ...
    const normalizedClubRole = String(clubRoleValue || '')
      .toLowerCase()
      .replace(/_/g, '-');
    const isAdminRole =
      rawRole === 'admin' ||
      rawRole === 'super-admin' ||
      rawRole === 'super_admin' ||
      normalizedProfile.isAdmin === true;
    // Club admin if explicitly flagged on profile or via clubRoles for current club
    const isClubAdminRole =
      !isAdminRole &&
      (rawRole === 'club-admin' ||
        normalizedClubRole === 'club-admin' ||
        normalizedClubRole === 'admin');
    ...
    const isOwnPlayer = !!(
      player &&
      user &&
      (player.linkedAccountId === user.uid || player.linkedUserId === user.uid)
    );
    ...
    normalizedProfile.isAdmin,
    player && player.id,
    player && player.clubId,
    player && (player.linkedAccountId || player.linkedUserId),
    clubId,
    clubRoleValue,
  ]);
