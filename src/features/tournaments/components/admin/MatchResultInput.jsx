...
const MatchResultInput = ({ clubId, tournamentId, match, onResultSubmitted }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
      {/* Match Info */}
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Inserisci Risultato</h3>
        <div className="flex items-center justify-between text-sm">
          <span className="text-gray-700 dark:text-gray-300 font-medium">{match.team1Name || 'Squadra 1'}</span>
          <span className="text-gray-500 dark:text-gray-400 mx-2">vs</span>
          <span className="text-gray-700 dark:text-gray-300 font-medium">{match.team2Name || 'Squadra 2'}</span>
        </div>
        {match.groupName && (
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Girone: {match.groupName}</p>
        )}
        {match.round && (
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {success && (
          <div className="p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
            <p className="text-sm text-green-600 dark:text-green-400">✓ Risultato salvato con successo!</p>
          </div>
        )}
